...
@router.post("/guardar_inventario")
async def guarda_datos(input: Request, token: str =  Depends(oauth2_scheme_2)):
    ...
    # else:
            ...
            ruta = DIRECION
            dirercion_archvios = '{}tipo_docker_inventarios/inventarios/fotos'.format(ruta)
            ...
            dirercion_archvios_2 = '{}tipo_docker_inventarios/inventarios/fotos/'.format(ruta)
            ...
            dato={
                "respuesta":"Inventario Guardado con Exito",
                "listado_inventarios":documento_inf[1], 
            }
            # dato={
...
@router.post("/eliminar_inventarios")
async def guarda_datos(input: Request, token: str =  Depends(oauth2_scheme_2)):
...
@router.post("/eliminar_cargo_inventario")
async def guarda_datos(input: Request, token: str =  Depends(oauth2_scheme_2)):
...
@router.post("/select_cargo_inventario")
async def guarda_datos(input: Request, token: str =  Depends(oauth2_scheme_2)):
            ...
            dato={
                "respuesta":"----",
                "asesorios_inventarios":documento_inf[0], 
                "observaciones_inventarios":documento_inf[1], 
            }
            # dato={
            #     "respuesta":"Usuario Guardado con Exito",
            # }
...
@router.post("/guardar_cargo_inventarios")
async def guarda_datos(input: Request, token: str =  Depends(oauth2_scheme_2)):
...
if __name__ == "__main__":
     config = uvicorn.Config("inventarios:app", port=PUERTO+37, host = IP, log_level="info", reload=True)
