...
def ingreso_anotacion_operacional(datos):
    ...
    hora_evento = transformar(hora_evento)
    ...
    tipo_fuerza = transformar(tipo_fuerza)
    ...
    dato="""('{}',	'{}',	'{}',	'{}',	'{}',	'{}',	'{}',	'{}',	'{}',	'{}',	'{}',	'{}',	'{}',	'{}',	'{}',	'{}',	'{}',	'{}',	'{}',	'{}',	'{}',	'{}',	'{}',	'{}',	'{}',	'{}',	'{}',	'{}',	'{}',	'{}',	'{}')""".format(fecha_evento,	hora_evento,	departamento,	municipio,	latitud,	gr_n,	m_n,	s_n,	longitud,	gr_l,	m_l,	s_l,	hecho,	resumen,	enemigo,	tipo_evento,	tipo_fuerza,	divi_padre,	cp,	pel,	divi_hija,	brigada,	batallon,	sitio,	estructura,	hecho_nuevo,	 coordenadas_x, coordenadas_y,	color,	fecha_hora_militar,	color_titulo)
    query="insert into afectaciones_q5 (fecha_evento,	hora_evento,	departamento,	municipio,	latitud,	gr_n,	m_n,	s_n,	longitud,	gr_l,	m_l,	s_l,	hecho,	resumen,	enemigo,	tipo_evento,	tipo_fuerza,	divi_padre,	cp,	pel,	divi_hija,	brigada,	batallon,	sitio,	estructura,	hecho_nuevo,	x,	y,	color,	fecha_hora_militar,	color_titulo) values"+dato
    ...
    query_select = "SELECT id FROM afectaciones_q5 ORDER BY id DESC LIMIT 1;"
    ...
    for x in resultado_reistrar:
        ...
        grd = str(resultado_reistrar[numero]).upper()
        nomb = str(resultado_reistrar[numero_b]).upper()
        query_res = "('"+str(id_q5[0])+"', '"+fecha_evento+"', '"+str(res)+"', '"+str(grd)+"', '"+str(nomb)+"')"
        if query_res_2 !="":
            query_res_2 =  query_res_2 +","+query_res
        else:
            query_res_2 = query_res

        if numero_b < indice-1:
            numero = numero +3
            numero_a = numero_a +3
            numero_b = numero_b +3
        else:
            break
    query_2="insert into resultado_afectacion_q5 (id_q5_afectacion, fecha_resultado, resultado, grd, nombre) values "+query_res_2
    cursor.execute(query_2)
    conn.commit()
...
def anotaciones_registro_boletines(datos):
    ...
    query="select * from afectaciones_q5  order by fecha_evento desc"
