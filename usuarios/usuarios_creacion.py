...
async def guarda_datos(input: Request, token: str =  Depends(oauth2_scheme_2)):
    ...
    # else:
            ...
            ruta = DIRECION
            dirercion_archvios = '{}usuarios/fotos'.format(ruta)
            ...
            documento_inf  = guardar_evento(contents, foto_name, acta_reserva_file_nombres)
            ...
            if acta_reserva_2 =="---":
                raiz= "tipo_docker_eventos/a_a_guardar_evento/documentos".format(ruta)
                dirercion_archvios_2 = '{}usuarios/documentos/'.format(ruta)
                # arcivos a la ruta final 
...
if __name__ == "__main__":
     config = uvicorn.Config("usuarios_creacion:app", port=PUERTO+200, host = IP, log_level="info", reload=True)
