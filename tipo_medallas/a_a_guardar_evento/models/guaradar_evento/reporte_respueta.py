...
def informe_pendiente(datos):
    ...
    if permiso == "EJC":
        query="select * from asignacion_plazos  where respuesta = 'SI'   ORDER BY  fecha_plazo_respuesta ASC"
    else:
        query="select * from asignacion_plazos where cargo LIKE '%{}%' AND respuesta = 'SI'  ORDER BY  fecha_plazo_respuesta ASC".format(permiso)
    ...
    for data_x in data:
        if data_x[48] != "SI":
            ahora = datetime.now()
            fin_anio_2021 = datetime.strptime(data_x[38], "%Y-%m-%d")
    ...
    for data_x in nuevo_listados:
        ...
        if data_x[48] != "SI":
            ...
            pdf.multi_cell(40, 5, str(data_x[38]), 0, "J", False)
            ...
            if "Vencidos" in str(dato):
                ...
                pdf.multi_cell(35, 5, str(data_x[50]), 0, "J", True)
                ...
                pdf.set_text_color(0,0,0)
                pdf.set_font('Calibri', 'B', 10)
                pdf.multi_cell(35, 5, str(data_x[50]), 0, "J", False)
            ...
            pdf.line(10, posicion_final, 266, posicion_final)
...
def informe_pendiente_estadistica(datos):
    ...
    if permiso == "EJC":
        query="select * from asignacion_plazos  where respuesta = 'SI'   ORDER BY  cargo ASC"
    else:
        query="select * from asignacion_plazos where cargo LIKE '%{}%' AND respuesta = 'SI'  ORDER BY  cargo ASC".format(permiso)

        
    if permiso == "EJC":
        query_2="select * from asignacion_plazos  where respuesta = 'SI'   ORDER BY  fecha_plazo_respuesta ASC"
    else:
        query_2="select * from asignacion_plazos where cargo LIKE '%{}%' AND respuesta = 'SI'  ORDER BY  fecha_plazo_respuesta ASC".format(permiso)
    ...
    for data_x in nuevo_listados:
        ...
        if data_x[48] != "SI":
            if data_x[50] not in dias_vencidos:
                dias_vencidos.append(data_x[50])
    ...
    for data_x in data_2:
        ...
        if data_x[40] != "SI":
            if data_x[38] not in dias:
                dias.append(data_x[38])
    ...
    plt.title("PLAZOS SIN RESPUESTA DE LAS DEPENDENCIAS", fontdict = font1)
    ...
    pdf.image("C:/Users/nicolas.valbuena/Documents/programacion 2023/server/tipo_plazos/a_a_guardar_evento/informe_pendiente_qr/Ejemplo1.jpg",5,30,150,110)
    ax.get_figure().clf()
    ...
    plt.title("CANTIDAD DE PLAZOS VENCIDOS", fontdict = font1)
    ...
    pdf.image("C:/Users/nicolas.valbuena/Documents/programacion 2023/server/tipo_plazos/a_a_guardar_evento/informe_pendiente_qr/dias.jpg",155,30,125,110)
    ax_2.get_figure().clf()
    ...
    plt.title("PLAZOS SIN RESPUESTA POR FECHA", fontdict = font1)
    plt.xlabel("Cantidad", fontdict = font2)
    ...
    pdf.image("C:/Users/nicolas.valbuena/Documents/programacion 2023/server/tipo_plazos/a_a_guardar_evento/informe_pendiente_qr/dias_vencidos.jpg",10,138,150,70)
    ax_3.get_figure().clf()

    suma_de_notas = sum(cantidad)
    pdf.rounded_rect(195, 155, 70, 50, 1,'D', '1234')
    pdf.set_font('BebasNeue', '', 50)
    pdf.text(225,180,str(suma_de_notas))
    pdf.set_font('BebasNeue', '', 20)
    pdf.text(210,190,"CANTIDAD DE PLAZOS")
