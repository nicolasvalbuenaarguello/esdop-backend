...
def estadistica_resultados_mineria(datos_res, hechos):
    ...
    #afectacion_mineria = datos_res
    #rme
    rme_mineria = list(filter(lambda afectacion_mineria: 'RME' in afectacion_mineria[9] , afectacion_mineria))
    #EIYM
    eiym = list(filter(lambda hechos :  hechos[0] == "EXPLORACIÓN Y EXPLOTACIÓN ILÍCITA" , hechos))#se debe mejorar con la actualizacion del sicoe
    #upm
    upm = list(filter(lambda afectacion_mineria: 'UPM ILEGAL' in afectacion_mineria[12] or 'UPM LEGAL' in afectacion_mineria[12] or 'SOCAVON' in afectacion_mineria[12] or 'BOCAMINA' in afectacion_mineria[12], afectacion_mineria))#organizado
    #maquinaria pesada
    ...
    #dragas
    dragas = list(filter(lambda afectacion_mineria: 'DRAGA(S)' in afectacion_mineria[12]  , afectacion_mineria))
    dragones = list(filter(lambda afectacion_mineria: 'DRAGON(ES)' in afectacion_mineria[12] , afectacion_mineria)) 
    oro= list(filter(lambda afectacion_mineria: 'ORO' in afectacion_mineria[12] , afectacion_mineria)) 
    mercurio= list(filter(lambda afectacion_mineria: 'MERCURIO' in afectacion_mineria[12] , afectacion_mineria)) 
    cianuro= list(filter(lambda afectacion_mineria: 'CIANURO' in afectacion_mineria[12] , afectacion_mineria)) 
    ...
    return[ capturas_mineria, rme_mineria, eiym, excavadoras, retroexcavadoras, maquinaria_pesada, buldocer, upm, dragas,  motores, combustibles_mineria, capturas_sin_delco,  maquinaria_amarilla, explosivos_mineria_kg, explosivos_mineria_m, explosivos_mineria_und, material_transporte, dragones, oro, mercurio, cianuro]
...
def calculo_doble(datos_res, parametro, parametro_dos, identificador, identificador_dos, identificador_suma):
    ...
    numero = formato_numero(numero)
    return numero
def porcentajes(numero_ant, numero_act, constante=1):
    """
    Calcula el porcentaje de variación entre dos valores.
    
    - numero_ant: valor anterior
    - numero_act: valor actual
    - constante: factor de ajuste multiplicativo (por defecto = 1)
    Devuelve: porcentaje formateado con formato_numero()
    """

    try:
        numero_ant = float(numero_ant)
        numero_act = float(numero_act)
        constante = float(constante)
    except (TypeError, ValueError):
        return 0  # Si alguno no es numérico

    if numero_ant > 0:
        if numero_act > 0:
            porcentaje = ((numero_act - numero_ant) / numero_ant) * 100
        else:
            porcentaje = -100
    elif numero_act > 0:
        porcentaje = 100
    ...
    return formato_numero(porcentaje)
def calcular_direccion(oro_actual, oro_anterior):
    # Limpia los valores (por si vienen como '1.135')
    try:
        act = float(str(oro_actual).replace('.', '').replace(',', '.'))
        ant = float(str(oro_anterior).replace('.', '').replace(',', '.'))
    except ValueError:
        return None  # Si algo no se puede convertir
...
def calculo_total_sin_filtro(datos_res, identificador_suma):
    """
    Calcula la suma total de una columna por índice o nombre.
    Soporta DataFrames o listas de listas/tuplas.
    """
    def convertir_a_numero(valor):
        try:
            # Limpia comas, espacios y convierte a float
            return float(str(valor).replace(',', '').strip())
        except (ValueError, TypeError):
            return 0.0

    if isinstance(datos_res, pd.DataFrame):
        # Identifica si el argumento es un índice o un nombre de columna
        if isinstance(identificador_suma, int):
            col = datos_res.iloc[:, identificador_suma]
        else:
            col = datos_res[identificador_suma]
        total = pd.to_numeric(col, errors='coerce').fillna(0).sum()
    else:
        # Si es lista de listas o tuplas
        valores = [
            convertir_a_numero(x[identificador_suma])
            for x in datos_res
            if len(x) > identificador_suma
        ]
        total = sum(valores)

    return formato_numero(total)
...
def calculo_total_sin_filtro_b(datos_res, identificador_suma):
    ...
    return formato_numero(numero_ant)
...
def encabezado_tabla_resultados_mapa(pdf, text):
    ...
    pdf.ln(20)
    pdf.cell(176)
    pdf.cell(145,10,text,1,0, 'C',True)
    pdf.ln(10)
    pdf.cell(176)
    pdf.cell(65,10,"INDICADOR",1,0, 'C',True)
    pdf.cell(40,10,"CANTIDAD",1,0, 'C',True)
    pdf.cell(40,10,"INDICADOR",1,0, 'C',True)
#tabla de resultados 
def tabla_resultados_con_mapa(pdf,text,datos_res_act, unidad, suma):
    ...
    pdf.cell(65,10,text,1,0, 'L',False)
    pdf.set_font('Arial', '', 12)
    pdf.cell(40,10,res[0],1,0, 'C',False)
    pdf.cell(40,10,unidad,1,0, 'C',False)
