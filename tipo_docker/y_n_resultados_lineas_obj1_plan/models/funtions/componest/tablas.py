...
def tabla_comparativa_linea(pdf, datos, datos_ant, id_enemigo, amenaza, suma, constante, cel, fill, texto, color, indicadores):
    ...
    num = 0
    if datos_res[0]!= "-" or  datos_res[1] !="-":
        pdf.cell(cel+20+10)
        pdf.cell(60,5,texto,1,0, 'L',fill)
        pdf.cell(30,5,datos_res[0],1,0, 'C',fill)
        pdf.cell(30,5,datos_res[1],1,0, 'C',fill)
        if datos_res[4] < 0:
        ...
        num = 1
    return [indicadores, num]

def tabla_comparativa_linea_2(pdf, datos, datos_ant, suma, constante, cel, fill, texto, color, indicadores, numeros):

    datos_res = calculo_comprativo_total(datos, datos_ant,  suma, constante)
    pdf.cell(cel)
    pdf.cell(10,5,str(numeros),1,0, 'C',fill)
    pdf.cell(80,5,texto,1,0, 'L',fill)
    pdf.cell(30,5,datos_res[0],1,0, 'C',fill)
    pdf.cell(30,5,datos_res[1],1,0, 'C',fill)
    #print(datos_res[1])
    if datos_res[5] < 0:
    ...
    if texto != "Combates sin resultados":
        if datos_res[6] < 0:
            pdf.set_text_color(125,0,0)
        else:
            pdf.set_text_color(0,0,0)
...
def sepador_mapa_dos(pdf, fill, text, cell, ancho):
    ...
    #pdf.set_fill_color(240,240,240)
def validar_indicador_unidad(json, json_ruta):
        obj_2 = []
        texto = []
        constante = []
        suma = []
        ...
        with open(json_ruta, "rb") as read_file:
            ...
            for x in data:
                for y in data[x]:
                    ...
                    texto.append(y['VARIABLE'])
                    constante.append(int(y['constante']))
                    suma.append(int(y['suma']))
                    posicion.append(int(y['posicion']))
                    spoa.append(y['SPOA'])

        return [obj_2, texto, constante, suma, posicion, spoa]
def nombre_enemigo_reducir(x):
...
def encabezado_comparativo_mapa(pdf, fill, anio_act, anio_ant, cel):
    ...
    pdf.cell(30,5,str(anio_ant),1,0, 'C',fill)
    pdf.cell(30,5,str(anio_act),1,0, 'C',fill)
...
def encabezado_comparativo_mapa_2(pdf, fill, anio_act, anio_ant, cel):
    pdf.set_line_width(0.1)
    pdf.set_fill_color(255,255,255)
    pdf.set_draw_color(0, 0, 0)
    pdf.set_text_color(0,0,0)
    ...
    pdf.cell(cel)
    pdf.cell(10,5,"No.",1,0, 'C',fill)
    pdf.cell(80,5,"EVENTO",1,0, 'C',fill)
    pdf.cell(30,5,str(anio_ant),1,0, 'C',fill)
    pdf.cell(30,5,str(anio_act),1,0, 'C',fill)
    pdf.cell(25,5,"DIFERENCIA",1,0, 'C',fill)
    pdf.cell(25,5,"PORCENTAJE",1,0, 'C',fill)
...
def encabezado_comparativo_enemigo_dinamico(pdf, fill, datos, datos_ant, amenaza, cel, color, suma, constante, id_enemigo, texto, indicadores,  numeros, obj):
    pdf.set_line_width(0.1)
    pdf.set_fill_color(color[0],color[1],color[2])
    pdf.set_draw_color(0, 0, 0)
    pdf.set_text_color(0,0,0)
    ...
    for x in texto:
        indicadores_f = tabla_comparativa_linea(pdf, datos[numero], datos_ant[numero], id_enemigo, amenaza[0], suma, constante, cel, fill,texto[numero] , color, indicadores)
        indicadores = indicadores_f[0]
        if indicadores_f[1] > 0:

            texto_evaluado.append(indicadores_f[1])
        numero +=1
...
def encabezado_comparativo_enemigo_dinamico_2(pdf, fill, datos, datos_ant, cel, color, suma, constante, texto, indicadores, objectivos, obj, numeros):
    ...
    if objectivos !=[]:
        while i < len(datos):
            ...
            if objectivos[i] == obj:
                ...
                pdf.set_text_color(0,0,0)
                pdf.set_font('Arial', 'B', 9)
                if texto[numero] != "Combates sin resultados":
                    indicadores = tabla_comparativa_linea_2(pdf, datos[numero], datos_ant[numero], suma[numero], constante[numero], cel, fill,texto[numero] , color, indicadores, numeros)
                else:
                    tabla_comparativa_linea_2(pdf, datos[numero], datos_ant[numero], suma[numero], constante[numero], cel, fill,texto[numero] , color, indicadores, numeros)
                
                textos_len.append(obj)
