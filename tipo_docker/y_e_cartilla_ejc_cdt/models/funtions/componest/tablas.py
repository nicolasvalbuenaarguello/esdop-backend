...
def estadistica_resultados(datos_res, hechos, filtro):
    ...
    #petroleo
    petroleo = list(filter(lambda datos_res: "PETROLEO" in datos_res[12], datos_res))
    ...
    return[rme, presentacion_voluntaria, sometimientos, capturas, mdom, asesinados, heridos, armas_largas, armas_cortas, armas_acompaniamiento, municiones, combates_pos, comates_neg, combates_sin, combates, neutralizacion_terrorista, ae, map_anti_perosna, explosivos_kg, cordon,mecha, cocaina, marihuana, pbc, lab_cocaina, lab_pbc, semilleros, matas_coca, insumos_liquidos, insumos_solidos, capturas_artemisa, plantulas_sembradas, madera, capturas_mineria, rme_mineria, eiym, excavadoras, retroexcavadoras, maquinaria_pesada, buldocer, upm, dragas, liberados, rescatados, valvula, refineria, depositos, cocaina_proceso, pbc_proceso, capturas_contrabando, gasolina_contrabando, acpm_contrabando, insumos_liquidos_contrabando, vehiculos_contrabando, animales_incautados_contrabando, elementos_escolares_contrabando, jugueteria_contrabando,prendas_vestir_contrabando, hechos_contrabando, motores, combustibles_mineria, capturas_sin_delco, combustibles_contrabando, especies_aninales, ataque_fuerza, coltan, hostigamientos, asonadas, basuco, herohina, drogas_sinteticas, lab_heroina, medios_lanzamientos, detonador_ini_electrico, detonador_electrico, dolares, pesos_colombianos, euros, explosivos_m, explosivos_und, artefactos_explosivos, maquinaria_amarilla, explosivos_mineria_kg, explosivos_mineria_m, explosivos_mineria_und, material_transporte, oleoducto, petroleo, piscina, combustibles_narcotrafico, inmuebles, muebles, vehiculos]
...
def formato_numero_numero_mas(numero):
    if numero !=0:
        if numero > 0:
...
def calculo_comprativo_total(datos_res_ant, datos_res_act, identificador_suma, constante):
    ...
    porcentaje_f = formato_numero(porcentaje)
    
    return [numero_ant, numero_act, porcentaje_f, diferencia, porcentaje]
...
#funcion que dibuja las divisiones en cuadro del  boletin del coe 
def tabla_boletin_coe(pdf, datos_res, text):
    pdf.set_fill_color(227, 227, 211)
    pdf.set_font('Calibri', 'B', 9)
    pdf.set_text_color(0,0,0)
    pdf.ln(5)
    pdf.cell(-5)
    pdf.cell(50,5,text,1,0, 'L',True)
    pdf.set_font('Calibri', 'B', 9)

    total = 0
    numero = calculo(datos_res, "DIV01", 1, 18)
    pdf.cell(15,5,numero,1,0, 'C',False)
    
    numero = calculo(datos_res, "DIV02", 1, 18)
    pdf.cell(15,5,numero,1,0, 'C',False)
       
    numero = calculo(datos_res, "DIV03", 1, 18)
    pdf.cell(15,5,numero,1,0, 'C',False)
       
    numero = calculo(datos_res, "DIV04", 1, 18)
    pdf.cell(15,5,numero,1,0, 'C',False)
       
    numero = calculo(datos_res, "DIV05", 1, 18)
    pdf.cell(15,5,numero,1,0, 'C',False)
       
    numero = calculo(datos_res, "DIV06", 1, 18)
    pdf.cell(15,5,numero,1,0, 'C',False)
       
    numero = calculo(datos_res, "DIV07", 1, 18)
    pdf.cell(15,5,numero,1,0, 'C',False)
       
    numero = calculo(datos_res, "DIV08", 1, 18)
    pdf.cell(15,5,numero,1,0, 'C',False)
           
    numero = calculo(datos_res, "DAVAA", 1, 18)
    pdf.cell(15,5,numero,1,0, 'C',False)
           
    numero = calculo(datos_res, "DIVFE", 1, 18)
    pdf.cell(15,5,numero,1,0, 'C',False)
               
    numero = calculo(datos_res, "FUTCO", 1, 18)
    pdf.cell(15,5,numero,1,0, 'C',False)
               
    numero = calculo(datos_res, "FTCEC", 1, 18)
    pdf.cell(15,5,numero,1,0, 'C',False)
    ...
    numero = calculo_total(datos_res, 1, 18)
    pdf.cell(20,5,numero,1,0, 'C',True)
    
  
    pdf.set_fill_color(81, 60, 49)
    numero = calculo_tres(datos_res,"GAO-RESIDUAL","GAO-r disidencias FARC","GAO - Residual Disidencias FARC", 7, 18)                 
    pdf.cell(16,5,numero,1,0, 'C',True)

    pdf.set_fill_color(0, 176, 80)
    numero = calculo(datos_res, "GDO", 7, 18)
    pdf.cell(16,5,numero,1,0, 'C',True)
      
    pdf.set_fill_color(188, 16, 23)                 
    numero = calculo(datos_res, "GAO ELN", 7, 18)
    pdf.cell(16,5,numero,1,0, 'C',True)
          
    pdf.set_fill_color(36, 54, 41)                 
    numero = calculo(datos_res, "GAO CLAN DEL GOLFO", 7, 18)
    pdf.cell(16,5,numero,1,0, 'C',True)
          
    pdf.set_fill_color(255, 192, 0)                 
    numero = calculo(datos_res, "NARCOTRÁFICO", 7, 18)
    pdf.cell(16,5,numero,1,0, 'C',True)
              
    pdf.set_fill_color(166, 166, 166)                 
    numero = calculo(datos_res, "DELINCUENCIA", 7, 18)
    pdf.cell(16,5,numero,1,0, 'C',True)


#funcion que dibuja las divisiones en cuadro del  boletin del coe 
def tabla_boletin_coe_regiones(pdf, datos_res, text, identificador, identificador_suma):
    pdf.set_fill_color(227, 227, 211)
    pdf.set_font('Calibri', 'B', 12)
    pdf.set_text_color(0,0,0)
    pdf.ln(5)
    pdf.cell(-5)
    pdf.cell(70,5,text,1,0, 'L',True)
    pdf.set_font('Calibri', 'B', 10)
    
    total = 0

    region = ["AMAZONAS", "CAQUETÁ", "GUAINÍA", "GUAVIARE", "PUTUMAYO", "VAUPÉS"]
    numero = calculo_por_lista(datos_res, region, identificador, identificador_suma)
    pdf.cell(23,5,numero,1,0, 'C',False)
    
    region = ["ANTIOQUIA", "BOGOTÁ, D. C.", "BOGOTÁ- D. C.", "BOYACÁ", "CALDAS", "CUNDINAMARCA", "HUILA", "NORTE DE SANTANDER", "QUINDÍO", "RISARALDA", "SANTANDER", "TOLIMA" ]

    numero = calculo_por_lista(datos_res, region, identificador, identificador_suma)
    pdf.cell(23,5,numero,1,0, 'C',False)

    region = ["ARCHIPIÉLAGO DE SAN ANDRÉS, PROVIDENCIA Y SANTA CATALINA","ARCHIPIÉLAGO DE SAN ANDRÉS- PROVIDENCIA Y SANTA CATALINA", "ATLÁNTICO", "BOLÍVAR", "CESAR", "CÓRDOBA", "LA GUAJIRA", "MAGDALENA", "SUCRE"]
    numero = calculo_por_lista(datos_res, region, identificador, identificador_suma)
    pdf.cell(23,5,numero,1,0, 'C',False)
   
       
    region = ["ARAUCA", "CASANARE", "META", "VICHADA"]
    numero = calculo_por_lista(datos_res, region, identificador, identificador_suma)
    pdf.cell(23,5,numero,1,0, 'C',False)
       
    region = ["CAUCA", "CHOCÓ", "NARIÑO", "VALLE DEL CAUCA"]
    numero = calculo_por_lista(datos_res, region, identificador, identificador_suma)
    pdf.cell(23,5,numero,1,0, 'C',False)
       
    numero = calculo_total(datos_res, 1, identificador_suma)
    pdf.cell(23,5,numero,1,0, 'C',False)
    ...
    pdf.set_fill_color(81, 60, 49)
    numero = calculo_tres(datos_res,"GAO-RESIDUAL","GAO-r disidencias FARC","GAO - Residual Disidencias FARC", 7, 18)                 
    pdf.cell(23,5,numero,1,0, 'C',True)
...
#funcion que dibuja las divisiones en cuadro del  boletin del coe hechos
def tabla_boletin_coe_hechos(pdf, datos_res, text):
    pdf.set_fill_color(227, 227, 211)
    pdf.set_font('Calibri', 'B', 9)
    pdf.set_text_color(0,0,0)
    pdf.ln(5)
    ...
    total = 0
    numero = calculo(datos_res, "DIV01", 2, 14)
    pdf.cell(15,5,numero,1,0, 'C',False)
    
    numero = calculo(datos_res, "DIV02", 2, 14)
    pdf.cell(15,5,numero,1,0, 'C',False)
       
    numero = calculo(datos_res, "DIV03", 2, 14)
    pdf.cell(15,5,numero,1,0, 'C',False)
       
    numero = calculo(datos_res, "DIV04", 2, 14)
    pdf.cell(15,5,numero,1,0, 'C',False)
       
    numero = calculo(datos_res, "DIV05", 2, 14)
    pdf.cell(15,5,numero,1,0, 'C',False)
       
    numero = calculo(datos_res, "DIV06", 2, 14)
    pdf.cell(15,5,numero,1,0, 'C',False)
       
    numero = calculo(datos_res, "DIV07", 2, 14)
    pdf.cell(15,5,numero,1,0, 'C',False)
       
    numero = calculo(datos_res, "DIV08", 2, 14)
    pdf.cell(15,5,numero,1,0, 'C',False)
           
    numero = calculo(datos_res, "DAVAA", 2, 14)
    pdf.cell(15,5,numero,1,0, 'C',False)
           
    numero = calculo(datos_res, "DIVFE", 2, 14)
    pdf.cell(15,5,numero,1,0, 'C',False)
               
    numero = calculo(datos_res, "FUTCO", 2, 14)
    pdf.cell(15,5,numero,1,0, 'C',False)
               
    numero = calculo(datos_res, "FTCEC", 2, 14)
    pdf.cell(15,5,numero,1,0, 'C',False)
    ...
    pdf.set_font('Calibri', 'B', 9)
    pdf.set_fill_color(166, 166, 166)
    numero = calculo_total(datos_res, 2, 14)
    pdf.cell(20,5,numero,1,0, 'C',True)
    
  
    pdf.set_fill_color(81, 60, 49)
    numero = calculo_tres(datos_res,"GAO-RESIDUAL","GAO-r disidencias FARC","GAO - Residual Disidencias FARC", 8, 14)                 
    pdf.cell(16,5,numero,1,0, 'C',True)

    pdf.set_fill_color(0, 176, 80)
    numero = calculo(datos_res, "GDO", 8, 14)
    pdf.cell(16,5,numero,1,0, 'C',True)
      
    pdf.set_fill_color(188, 16, 23)                 
    numero = calculo(datos_res, "GAO ELN", 8, 14)
    pdf.cell(16,5,numero,1,0, 'C',True)
          
    pdf.set_fill_color(36, 54, 41)                 
    numero = calculo(datos_res, "GAO CLAN DEL GOLFO", 8, 14)
    pdf.cell(16,5,numero,1,0, 'C',True)
          
    pdf.set_fill_color(255, 192, 0)                 
    numero = calculo(datos_res, "NARCOTRÁFICO", 8, 14)
    pdf.cell(16,5,numero,1,0, 'C',True)
              
    pdf.set_fill_color(166, 166, 166)                 
    numero = calculo(datos_res, "DELINCUENCIA", 8, 14)
    pdf.cell(16,5,numero,1,0, 'C',True)
...
def encabezado_comparativo_mapa(pdf, fill, anio_act, anio_ant):
    pdf.set_line_width(0.1)
    pdf.set_fill_color(193, 30, 38)
    pdf.set_draw_color(0, 0, 0)
    pdf.set_text_color(255,255,255)
    ...
    pdf.cell(176)
    pdf.cell(60,5,"RESULTADOS",1,0, 'C',fill)
    pdf.cell(40,10,str(anio_ant),1,0, 'C',fill)
    pdf.cell(40,10,str(anio_act),1,0, 'C',fill)
    ...
    pdf.ln(5)
    pdf.cell(176)
    pdf.cell(60,5,"EVENTO",1,0, 'C',fill)
...
def encabezado_comparativo(pdf, fill, fecha_anterior, fecha_actual, cell):
    ...
    pdf.cell(variable)
    pdf.cell(17.5,z,"DIF.",1,0, 'C',fill)
   
    #variable = variable + 35
    #pdf.cell(variable)
    pdf.cell(17.5,z,"%",1,0, 'C',fill)
    ...
    pdf.multi_cell(65, z-5, str("RESULTADOS"), 1, "L", fill)
    ...
    pdf.ln(-0)
    pdf.cell(cell-5)
    pdf.multi_cell(65, 5, str("EVENTO"), 1, "L", fill)
    pdf.set_fill_color(240, 240, 240)
    pdf.set_text_color(0,0,0)
    return z
...
def sepador_mapa(pdf, fill, text):
    ...
    #separado para el mapa
def sepador_mapa_dos(pdf, fill, text, cell):
    
    pdf.ln(5)
    pdf.cell(cell)
    pdf.set_text_color(0,0,0)
    pdf.set_fill_color(208,208,182)
    pdf.cell(160,5,text,1,0, 'C',fill)
    pdf.set_fill_color(240,240,240)
...
def claculo_mapa_dos(pdf, datos_res_ant,  datos_res_act, text, constante, id_enemigo, suma, cell):
    ...
    pdf.cell(cell)
    pdf.set_text_color(0,0,0)
    pdf.cell(65,5,text,1,0, 'L',True)

    resultados  = calculo_comprativo_total(datos_res_ant, datos_res_act, suma, constante)
    pdf.cell(30,5,str(resultados[0]),1,0, 'C',False)
    pdf.cell(30,5,str(resultados[1]),1,0, 'C',False)
    if resultados[3] < 0:
    ...
    pdf.cell(17.5,5,str(formato_numero(resultados[3])),1,0, 'C',True)
    if resultados[4] < 0:
        pdf.set_fill_color(125,0,0)
        pdf.set_text_color(255,255,255)
    else:
        pdf.set_fill_color(240,240,240)
        pdf.set_text_color(0,0,0)
    pdf.cell(17.5,5,str(resultados[2]),1,0, 'C',True)
    pdf.set_fill_color(240,240,240)
    pdf.set_text_color(0,0,0)

def claculo_maquinaria_amarialla_mapa_dos(pdf, datos_res_ant,  datos_res_act, text, constante, id_enemigo, suma, cell):
    pdf.ln(5)
    pdf.cell(cell)
    pdf.set_text_color(0,0,0)
    pdf.cell(65,5,text,1,0, 'L',True)
    ...
    diferencia = maquinari_amarialla_actual - maquinari_amarialla_anterior
    ...
    resultados_f = formato_numero(resultados)
    ...
    pdf.cell(17.5,5,str(formato_numero(diferencia)),1,0, 'C',True)
    if resultados < 0:
        pdf.set_fill_color(125,0,0)
        pdf.set_text_color(255,255,255)
    else:
        pdf.set_fill_color(240,240,240)
        pdf.set_text_color(0,0,0)
    pdf.cell(17.5,5,str(resultados_f),1,0, 'C',True)

def celda_afectaciones(pdf, ln, cel,text,rango, id, fill):
    pdf.ln(ln)
    pdf.cell(cel)
    ...
    for rang in range(rango) :
        ...
        if fill != True:
            if numero == valor_c:
                ...
                if text[valor_c]<0:
                    ...
                    pdf.set_fill_color(208, 208, 182)
                    pdf.set_text_color(0,0,0)
                numero_inverso = formato_numero_numero_mas(text[valor_c]*-1)
                valor_c = valor_c +4
            ...
            if numero == valor_c_1:
                ...
                if text[valor_c_1]<0:
                    ...
                    pdf.set_fill_color(208, 208, 182)
                    pdf.set_text_color(0,0,0)
                numero_inverso = str(formato_numero(text[valor_c_1] * -1))+" %"
                valor_c_1 = valor_c_1 +4

            if numero != 2:
                if numero_inverso != "":
                    texto = numero_inverso
                else:
                    texto = formato_numero(float(text[numero]))
            else:
                texto = text[numero]
        ...
        pdf.cell(text[0][rang],text[1][rang],str(texto),1,0, text[id[0]][rang],text[id[1]][rang])
        numero = numero +1
...
def calcular_comparativo_res(datos, text, indicador, suma, constante):
    ...
    diferencia_ofi = asesinados_oficiales[0] - asesinados_oficiales[1]
    diferencia_ofi_heridos = heridos_oficiales[0] - heridos_oficiales[1]
    ...
    diferencia =  total_ant_ofi - total_act_ofi

    total_ant_ofi =  total_ant_ofi
    total_act_ofi =  total_act_ofi
    porcentajes_total_ofi_f =  porcentajes_total_ofi

    return [asesinados_oficiales[0], asesinados_oficiales[1], diferencia_ofi, asesinados_oficiales[2], heridos_oficiales[0], heridos_oficiales[1], diferencia_ofi_heridos, heridos_oficiales[2],total_ant_ofi,total_act_ofi, diferencia, porcentajes_total_ofi_f, porcentajes_total_ofi]
...
def cuadro_heridos_asesinados_comparativos(pdf, datos, filtro):
    ...
    pdf.ln(30)
    pdf.cell(-5)
    pdf.cell(166.5,10,"AFECTACIONES POR ACCIÓN DIRECTA DEL ENEMIGO",1,0, 'C',True)
    ...
    celda_afectaciones(pdf, 10, -5, text, rango, id, True)
    pdf.set_fill_color(208, 208, 182)
    pdf.set_text_color(0,0,0)
    pdf.ln()
    pdf.cell(25)
    ...
    pdf.cell(11,5,str("DIF."),1,0, 'C',True)
    ...
    x = (30,11,11,11,12.5,11,11,11,12.5,11,11,11,12.5)
    y= (10,10,10,10,10,10,10,10,10,10,10,10,10)
    rango =13
    a = ("L","C","C","C","C","C","C","C","C","C","C","C","C") 
    fill = (False, False, False, True,True, False, False, True,True, False, False, True,True)
    id = (15,16)

    datos_oficial = calcular_comparativo_res(datos, "OFICIALES", 11, 18, -1)
 
    text = (x, y, "OFICIALES", datos_oficial[0], datos_oficial[1], datos_oficial[2], datos_oficial[3], datos_oficial[4], datos_oficial[5], datos_oficial[6], datos_oficial[7], datos_oficial[8], datos_oficial[9], datos_oficial[10], datos_oficial[11] , a, fill )
    
    celda_afectaciones(pdf, 5, -5, text, rango, id, False)

    datos_subocial = calcular_comparativo_res(datos, "SUBOFICIALES", 11, 18, -1)
    text = (x, y, "SUBOFICIALES", datos_subocial[0], datos_subocial[1], datos_subocial[2], datos_subocial[3], (datos_subocial[4]), datos_subocial[5], datos_subocial[6], datos_subocial[7], datos_subocial[8] , datos_subocial[9] , datos_subocial[10] , datos_subocial[11] , a, fill )
    
    celda_afectaciones(pdf, 10, -5, text, rango, id, False)


    datos_slp = calcular_comparativo_res(datos, "SLP", 19, 18, -1)
    text = (x, y, "SLP", datos_slp[0], datos_slp[1], datos_slp[2], datos_slp[3], datos_slp[4], datos_slp[5], datos_slp[6], datos_slp[7], datos_slp[8] , datos_slp[9] , datos_slp[10] , datos_slp[11] , a, fill )
    
    celda_afectaciones(pdf, 10, -5, text, rango, id, False)

    datos_slp_18 = calcular_comparativo_res_r(datos, "SOLDADOS", "SLP", 11, 19, 18, -1)
    text = (x, y, "SL18", datos_slp_18[0], datos_slp_18[1], datos_slp_18[2], datos_slp_18[3], datos_slp_18[4], datos_slp_18[5], datos_slp_18[6], datos_slp_18[7], datos_slp_18[8] , datos_slp_18[9] , datos_slp_18[10] , datos_slp_18[11] , a, fill )
    celda_afectaciones(pdf, 10, -5, text, rango, id, False)
    ...
    total_heridos_2022 = datos_oficial[4] + datos_subocial[4] + datos_slp[4] + datos_slp_18[4]
    total_heridos_2023 = datos_oficial[5] + datos_subocial[5] + datos_slp[5] + datos_slp_18[5]
    total_porcentaje_heridos = porcentajes(total_heridos_2022, total_heridos_2023,-1)

    diferencia_asesinados = total_asesinados_2022 - total_asesinados_2023
    diferencia_heridos = total_heridos_2022 - total_heridos_2023
    ...
    diferencia_total = total_2022 - total_2023
    ...
    fill = (True, True, True, True, True, True, True, True, True, True, True, True, True)

    text = (x, y, "TOTAL", total_asesinados_2022, total_asesinados_2023,diferencia_asesinados, total_porcentaje_asinados, total_heridos_2022, total_heridos_2023, diferencia_heridos, total_porcentaje_heridos, total_2022, total_2023, diferencia_total,  total_porcentaje , a, fill )
    celda_afectaciones(pdf, 10, -5, text, rango, id, False)
    ...
    celda_afectaciones(pdf, 10, 165, text, rango, id, True)
    ...
    if datos_eln[8] != 0 or datos_eln[9] != 0:
        text = (x, y, "GAO-ELN", datos_eln[0], datos_eln[1], datos_eln[2], datos_eln[3], datos_eln[4], datos_eln[5], datos_eln[6], datos_eln[7], datos_eln[8], datos_eln[9], datos_eln[10], datos_eln[11] , a, fill )
        celda_afectaciones(pdf, 10, 165, text, rango, id, False)

    datos_gaor = calcular_comparativo_res_t(datos, "GAO-RESIDUAL","GAO-r disidencias FARC","GAO - Residual Disidencias FARC", 7, 18, -1)
    if datos_gaor[8] != 0 or datos_gaor[9] != 0:
        text = (x, y, "GAO-r", datos_gaor[0], datos_gaor[1], datos_gaor[2], datos_gaor[3], datos_gaor[4], datos_gaor[5], datos_gaor[6], datos_gaor[7], datos_gaor[8], datos_gaor[9], datos_gaor[10], datos_gaor[11] , a, fill )
        celda_afectaciones(pdf, 10, 165, text, rango, id, False)
    ...
    if datos_cg[8] != 0 or datos_cg[9] != 0:
        text = (x, y, "GAO-CG", datos_cg[0], datos_cg[1], datos_cg[2], datos_cg[3], datos_cg[4], datos_cg[5], datos_cg[6], datos_cg[7], datos_cg[8] , datos_cg[9], datos_cg[10], datos_cg[11], a, fill )
        celda_afectaciones(pdf, 10, 165, text, rango, id, False)

    datos_cs = calcular_comparativo_res(datos, "GAO COMUNEROS DEL SUR", 7, 18, -1)

    if datos_cs[8] != 0 or datos_cs[9] != 0:
        text = (x, y, "GAO-CS", datos_cs[0], datos_cs[1], datos_cs[2], datos_cs[3], datos_cs[4], datos_cs[5], datos_cs[6], datos_cs[7], datos_cs[8] , datos_cs[9], datos_cs[10], datos_cs[11], a, fill )
        celda_afectaciones(pdf, 10, 165, text, rango, id, False)

    gdo = calcular_comparativo_res(datos, "GDO", 7, 18, -1)
    if gdo[8] != 0 or gdo[9] != 0:
        text = (x, y, "GDO", gdo[0], gdo[1], gdo[2], gdo[3], gdo[4], gdo[5], gdo[6], gdo[7], gdo[8] , gdo[9] , gdo[10] , gdo[11] , a, fill )
        celda_afectaciones(pdf, 10, 165, text, rango, id, False)
                

    gao_cap = calcular_comparativo_res(datos, "GAO CAPARROS", 7, 18, -1)
    if gao_cap[8] != 0 or gao_cap[9] != 0:
        text = (x, y, "GAO CAPARROS", gao_cap[0], gao_cap[1], gao_cap[2], gao_cap[3], gao_cap[4], gao_cap[5], gao_cap[6], gao_cap[7], gao_cap[8] , gao_cap[9], gao_cap[10], gao_cap[11], a, fill )
        celda_afectaciones(pdf, 10, 165, text, rango, id, False)
                        
    gao_pel = calcular_comparativo_res(datos, "GAO PELUSOS", 7, 18, -1)
    if gao_pel[8] != 0 or gao_pel[9] != 0:
        text = (x, y, "GAO PELUSOS", gao_pel[0], gao_pel[1], gao_pel[2], gao_pel[3], gao_pel[4], gao_pel[5], gao_pel[6], gao_pel[7], gao_pel[8], gao_pel[9], gao_pel[10], gao_pel[11] , a, fill )
        celda_afectaciones(pdf, 10, 165, text, rango, id, False)
                                
    narcotrafico = calcular_comparativo_res(datos, "NARCOTRÁFICO", 7, 18, -1)
    if narcotrafico[8] != 0 or narcotrafico[9] != 0:
        text = (x, y, "NARCOTRÁFICO", narcotrafico[0], narcotrafico[1], narcotrafico[2], narcotrafico[3], narcotrafico[4], narcotrafico[5], narcotrafico[6], narcotrafico[7], narcotrafico[8], narcotrafico[9], narcotrafico[10], narcotrafico[11] , a, fill )
        celda_afectaciones(pdf, 10, 165, text, rango, id, False)
    

    delco = calcular_comparativo_res(datos, "DELINCUENCIA", 7, 18, -1)
    if delco[8] != 0 or delco[9] != 0:
        text = (x, y, "DELINCUENCIA", delco[0], delco[1], delco[2], delco[3], delco[4], delco[5], delco[6], delco[7], delco[8], delco[9], delco[10], delco[11] , a, fill )
        celda_afectaciones(pdf, 10, 165, text, rango, id, False)


    delco_no = calcular_comparativo_res(datos, "NO APLICA", 7, 18, -1)
    if delco_no[8] != 0 or delco_no[9] != 0:
        text = (x, y, "NO APLICA", delco_no[0], delco_no[1], delco_no[2], delco_no[3], delco_no[4], delco_no[5], delco_no[6], delco_no[7], delco_no[8], delco_no[9], delco_no[10], delco_no[11] , a, fill )
        celda_afectaciones(pdf, 10, 165, text, rango, id, False)


    total_asesinados_2022 = datos_eln[0] + datos_gaor[0] + datos_cg[0] + gdo[0] + gao_cap[0] + gao_pel[0] + narcotrafico[0] + delco[0] + delco_no[0]
    total_asesinados_2023 = datos_eln[1] + datos_gaor[1] + datos_cg[1] + gdo[1] + gao_cap[1] + gao_pel[1] + narcotrafico[1] + delco[1] +  delco_no[1]

    diferencia_asesinados = total_asesinados_2022 - total_asesinados_2023
    total_porcentaje_asinados = porcentajes(total_asesinados_2022, total_asesinados_2023,-1)

    total_heridos_2022 = datos_eln[4] + datos_gaor[4] + datos_cg[4] + gdo[4] + gao_cap[4] + gao_pel[4] + narcotrafico[4] + delco[4] + delco_no[4]
    total_heridos_2023 = datos_eln[5] + datos_gaor[5] + datos_cg[5] + gdo[5] + gao_cap[5] + gao_pel[5] + narcotrafico[5] + delco[5] + delco_no[5]
    diferencia_heridos = total_heridos_2022 - total_heridos_2023
    total_porcentaje_heridos = porcentajes(total_heridos_2022, total_heridos_2023,-1)
...
def encabezado_interdicion(pdf):
    ...
    pdf.ln(6)
    pdf.cell(155)
    pdf.cell(160,10,"RESULTADOS INTERDICCIÓN",1,0, 'C',True)
    pdf.set_fill_color(208, 208, 182)
    pdf.set_text_color(0,0,0)
    pdf.ln(10)
    pdf.cell(155)
    pdf.cell(60,10,"INDICADOR",1,0, 'C',True)
...
def calculo_de_interdion(pdf,text, datos_res_act, meta, unidad):
    ...
    pdf.set_font('Arial', 'B', 12)
    pdf.cell(60,10,text,1,0, 'L',False)
...
def encabezado_tabla_resultados_mapa_compa(pdf, text, anio_act, anio_ant):
    ...
    pdf.ln(20)
    pdf.cell(147)
    pdf.cell(180,10,text,1,0, 'C',True)
    pdf.set_fill_color(208,208,182)
    pdf.set_text_color(0,0,0)
    pdf.ln(10)
    pdf.cell(147)
    pdf.cell(55,10,"RESULTADO",1,0, 'C',True)
    pdf.cell(25,10,str(anio_ant),1,0, 'C',True)
    pdf.cell(25,10,str(anio_act),1,0, 'C',True)
    pdf.cell(25,10,"DIF.",1,0, 'C',True)
    pdf.cell(25,10,"%",1,0, 'C',True)
    pdf.cell(25,10,"INDICADOR",1,0, 'C',True)
...
def tabla_resultados_con_mapa_comp(pdf,text,datos_res_ant, datos_res_act, unidad, suma, constante):
    ...
    pdf.set_font('Arial', 'B', 11)
    pdf.cell(55,10,text,1,0, 'L',False)
    pdf.set_font('Arial', '', 11)
    pdf.cell(25,10,str(res[0]),1,0, 'C',False)
    pdf.cell(25,10,str(res[1]),1,0, 'C',False)
    if res[3] < 0:
        pdf.set_fill_color(125,0,0)
        pdf.set_text_color(255,255,255)
    else:
            pdf.set_fill_color(208,208,182)
            pdf.set_text_color(0,0,0)
    pdf.cell(25,10,str(formato_numero(res[3])),1,0, 'C',True)
    ...
    pdf.cell(25,10,str(res[2]),1,0, 'C',True)
...
#funcion para titulos de resutados 
    ...
    diferecia = maquinari_amarialla_actual - maquinari_amarialla_anterior
    # resultados  = calculo_comprativo_total(maquinari_amarialla_anterior, maquinari_amarialla_actual, suma, constante)
    resultados = porcentajes(maquinari_amarialla_anterior, maquinari_amarialla_actual, constante)
    ...
    pdf.cell(55,10,text,1,0, 'L',False)
    pdf.set_font('Arial', '', 11)
    pdf.cell(25,10,str(maquinari_amarialla_anterior),1,0, 'C',False)
    pdf.cell(25,10,str(maquinari_amarialla_actual),1,0, 'C',False)
    ...
    pdf.cell(25,10,str(formato_numero(diferecia)),1,0, 'C',True)
    ...
    pdf.set_fill_color(208,208,182)
    pdf.set_text_color(0,0,0)
    pdf.cell(25,10,str(resultados_f),1,0, 'C',True)
    pdf.cell(25,10,unidad,1,0, 'C',False)
...
def cuadro_afectaciones(pdf, datos, filtro):
    ...
    pdf.cell(18,10,"EVENTO",1,0, 'C',True)
    pdf.cell(56,10,"ASESINADOS",1,0, 'C',True)
    pdf.cell(56,10,"HERIDOS",1,0, 'C',True)
    pdf.cell(56,10,"TOTAL NOVEDADES",1,0, 'C',True)
    pdf.set_text_color(0,0,0)
    pdf.set_fill_color(255,255,255)
    pdf.set_font('Arial', 'B', 8)
    pdf.ln()
    pdf.cell(147)
    pdf.cell(18,10,"AÑO",1,0, 'C',True)
    pdf.cell(14,10,datos[4],1,0, 'C',True)
    pdf.cell(14,10,datos[5],1,0, 'C',True)
    pdf.cell(14,10,str("DIF."),1,0, 'C',True)
    pdf.cell(14,10,str("%"),1,0, 'C',True)
    pdf.cell(14,10,datos[4],1,0, 'C',True)
    pdf.cell(14,10,datos[5],1,0, 'C',True)
    pdf.cell(14,10,str("DIF."),1,0, 'C',True)
    pdf.cell(14,10,str("%"),1,0, 'C',True)
    ...
    res_ase_ant = calculo_comprativo_total_sin_formato(datos[0],datos[1], 18, -1)

    var_ant = formato_numero(res_ase_ant[0])
    var_act = formato_numero(res_ase_ant[1])
    var_dife = formato_numero_numero_mas(res_ase_ant[1]-res_ase_ant[0])
    
    pdf.cell(14,10,str(var_ant),1,0, 'C',True)
    pdf.cell(14,10,str(var_act),1,0, 'C',True)
    ...
    por =  str(formato_numero(res_ase_ant[2]*-1)+" %")
    pdf.cell(14,10,str(var_dife),1,0, 'C',True)
    pdf.cell(14,10,str(por),1,0, 'C',True)

    pdf.set_draw_color(0, 0, 0)
    pdf.set_fill_color(255,255,255)
    pdf.set_text_color(0,0,0)

    res_heridos_ant = calculo_comprativo_total_sin_formato(datos[2],datos[3], 18, -1)
    var_ant = formato_numero(res_heridos_ant[0])
    var_act = formato_numero(res_heridos_ant[1])
    var_dife = formato_numero_numero_mas(res_heridos_ant[1]-res_heridos_ant[0])
    
    pdf.cell(14,10,str(var_ant),1,0, 'C',True)
    pdf.cell(14,10,str(var_act),1,0, 'C',True)
    

    if res_heridos_ant[2] < 0:
        pdf.set_draw_color(255, 0, 0)
        pdf.set_fill_color(125,0,0)
        pdf.set_text_color(255,255,255)
    else:
    ...
    por = str(formato_numero(res_heridos_ant[2]*-1)+" %")
    pdf.cell(14,10,str(var_dife),1,0, 'C',True)
    pdf.cell(14,10,str(por),1,0, 'C',True)
    ...
    total_ant = res_ase_ant[0] + res_heridos_ant[0]
    total_act = res_ase_ant[1] + res_heridos_ant[1]

    porcentaje= porcentajes( total_ant, total_act,-1 )

    var_dife = formato_numero_numero_mas(total_act - total_ant)
    total_ant = formato_numero(total_ant)
    total_act = formato_numero(total_act)

    pdf.cell(14,10,str(total_ant),1,0, 'C',True)
    pdf.cell(14,10,str(total_act),1,0, 'C',True)
    ...
    pdf.cell(14,10,str(var_dife),1,0, 'C',True)
    por = str(formato_numero(porcentaje*-1)+" %")
    pdf.cell(14,10,str(por),1,0, 'C',True)
    ...
    pdf.set_font('Arial', 'B', 12)
    pdf.image(asesinados,190,135,20,20)
    pdf.text(212,145,str(formato_numero(res_ase_ant[1]))+" ASESINADOS")
    pdf.image(heridos,260,135,20,20)
    pdf.text(282,145,str(formato_numero(res_heridos_ant[1]))+" HERIDOS")
    pdf.image(soldados,240,160,20,20)
    pdf.text(265,170,str(total_act)+" TOTAL AFECTACIONES")

    pdf.set_fill_color(94, 119, 89)
    pdf.rounded_rect(190, 40, 130, 30, 1,'F', '1234')
    pdf.set_text_color(255,255,255)
    pdf.set_font('Arial', 'B', 14)
    pdf.text(220,45,"FECHA INFORMACIÓN")
    pdf.set_font('Arial', 'B', 12)
...
def cuadro_menores_edad_ofico(pdf, datos_res):
    ...
    if rme_resul != "-":
        ...
        posicion = posi + posicion
        flecha = posi + flecha
        posi = cuadro_men_res_of(pdf, datos_res, "DELINCUENCIA","DELINCUENCIA","DELINCUENCIA","DELINCUENCIA", flecha)
        posicion = posi + posicion
        flecha = posi + flecha
    pdf.set_font('Arial', 'B', 10)
    pdf.ln(5)
...
def cuadro_enemigo_afectaciones_oficio(pdf, posicion_inicial, datos_res):
    ...
    if total != "-":
        ...
        posi = cuadro_afect_enemigo_oficio(pdf, datos_res, "DELINCUENCIA","DELINCUENCIA","DELINCUENCIA","DELINCUENCIA", flecha)
        posicion = posi + posicion
        flecha = posi + flecha
...
def cuadro_afectaciones_tropas_combate_oficio (pdf, posicion_inicial, datos_res ):
    ...
    if total != "-":
        ...
        posi = cuadro_afectaciones_tropas_oficio(pdf, datos_res, "DELINCUENCIA","DELINCUENCIA","DELINCUENCIA","DELINCUENCIA", flecha)
        flecha = posi + flecha
        posicion = posi + posicion
...
def cuadro_afect_enemigo_com(pdf, datos_res_ant, datos_res_act, enemigo, gao, gao_1, gao_2, ruta, flecha):

    pre_ant = calculo_tres_b(datos_res_ant[0], gao, gao_1, gao_2, 7, 18)
    some_ant = calculo_tres_b(datos_res_ant[1], gao, gao_1, gao_2, 7, 18)
    capt_ant = calculo_tres_b(datos_res_ant[2], gao, gao_1, gao_2, 7, 18)
    mdom_ant = calculo_tres_b(datos_res_ant[3], gao, gao_1, gao_2, 7, 18)
    
    pre_act = calculo_tres_b(datos_res_act[0], gao, gao_1, gao_2, 7, 18)
    some_act = calculo_tres_b(datos_res_act[1], gao, gao_1, gao_2, 7, 18)
    capt_act = calculo_tres_b(datos_res_act[2], gao, gao_1, gao_2, 7, 18)
    mdom_act = calculo_tres_b(datos_res_act[3], gao, gao_1, gao_2, 7, 18)
...
def cuadro_enemigo_afectaciones_comparativo(pdf,posicion_inicial, datos_res_ant, datos_res_actu, filtro, anio_act, anio_ant):
    ...
    ruta = filtro[15]
    flecha = 46.25
    pre_ant = calculo_total_sin_filtro_b(datos_res_ant[0], 18)
    some_ant = calculo_total_sin_filtro_b(datos_res_ant[1], 18)
    capt_ant = calculo_total_sin_filtro_b(datos_res_ant[2], 18)
    mdom_ant = calculo_total_sin_filtro_b(datos_res_ant[3], 18)

    pre_act = calculo_total_sin_filtro_b(datos_res_actu[0], 18)
    some_act = calculo_total_sin_filtro_b(datos_res_actu[1], 18)
    capt_act = calculo_total_sin_filtro_b(datos_res_actu[2], 18)
    mdom_act = calculo_total_sin_filtro_b(datos_res_actu[3], 18)
    ...
    if total_ant != "-" or total_act != "-":
        ...
        posi = cuadro_afect_enemigo_com(pdf, datos_res_ant, datos_res_actu, "GAO-ELN", "GAO ELN", "GAO ELN", "GAO ELN", ruta, flecha)
        posicion = posi + posicion
        flecha = flecha + posi
        posi = cuadro_afect_enemigo_com(pdf, datos_res_ant, datos_res_actu, "GAO-r", "GAO-RESIDUAL","GAO-r disidencias FARC","GAO - Residual Disidencias FARC", ruta, flecha)
        posicion = posi + posicion
        flecha = flecha + posi
        posi = cuadro_afect_enemigo_com(pdf, datos_res_ant, datos_res_actu, "GAO-CG", "GAO CLAN DEL GOLFO","GAO CLAN DEL GOLFO","GAO CLAN DEL GOLFO", ruta, flecha)
        posicion = posi + posicion
        flecha = flecha + posi

        posi = cuadro_afect_enemigo_com(pdf, datos_res_ant, datos_res_actu, "GAO-CAP", "GAO CAPARROS","GAO CAPARROS","GAO CAPARROS", ruta, flecha)
        posicion = posi + posicion  
        flecha = flecha + posi
              
        posi = cuadro_afect_enemigo_com(pdf, datos_res_ant, datos_res_actu,"GAO-PEL", "GAO PELUSOS","GAO PELUSOS","GAO PELUSOS", ruta, flecha)
        posicion = posi + posicion
        flecha = flecha + posi

        posi = cuadro_afect_enemigo_com(pdf, datos_res_ant, datos_res_actu, "GDO","GDO","GDO","GDO", ruta, flecha)
        posicion = posi + posicion
        flecha = flecha + posi

        posi = cuadro_afect_enemigo_com(pdf, datos_res_ant, datos_res_actu, "NARCO.","NARCOTRÁFICO","NARCOTRÁFICO","NARCOTRÁFICO", ruta, flecha)
        posicion = posi + posicion
        flecha = flecha + posi

        posi = cuadro_afect_enemigo_com(pdf, datos_res_ant, datos_res_actu, "DELCO","DELINCUENCIA","DELINCUENCIA","DELINCUENCIA", ruta, flecha)
        posicion = posi + posicion
        flecha = flecha + posi
        pdf.set_fill_color(217, 217, 217)
        pdf.set_font('Arial', 'B', 10)
...
#funcion para calcular las afectaciones del enemigo cuadro
def cuadro_afectaciones_tropas_comparativo(pdf, datos_res_afectaciones_ant, datos_res_afectaciones_act, enemigo, gao, gao_1, gao_2, ruta, flecha):

    ases_ant = calculo_tres_b(datos_res_afectaciones_ant[0], gao, gao_1, gao_2, 7, 18)
    ases_act = calculo_tres_b(datos_res_afectaciones_act[0], gao, gao_1, gao_2, 7, 18)

    heridos_ant = calculo_tres_b(datos_res_afectaciones_ant[1], gao, gao_1, gao_2, 7, 18)
    heridos_act = calculo_tres_b(datos_res_afectaciones_act[1], gao, gao_1, gao_2, 7, 18)
...
def cuadro_afectaciones_tropas_combate_comparativo (pdf, posicion_inicial, datos_res_afectaciones_ant, datos_res_afectaciones_act, filtro, anio_act, anio_ant ):
    ...
    asesinados_act = calculo_total_sin_filtro_b(datos_res_afectaciones_act[0], 18)
    heridos_act = calculo_total_sin_filtro_b(datos_res_afectaciones_act[1], 18)
    total_act_1 = asesinados_act + heridos_act 

    asesinados_ant = calculo_total_sin_filtro_b(datos_res_afectaciones_ant[0], 18)
    heridos_ant = calculo_total_sin_filtro_b(datos_res_afectaciones_ant[1], 18)
    ...
    if total_act != "-" or total_ant != "-":
        ...
        pos = cuadro_afectaciones_tropas_comparativo(pdf, datos_res_afectaciones_ant, datos_res_afectaciones_act, "GAO-ELN", "GAO ELN", "GAO ELN", "GAO ELN", ruta, flecha)
        flecha = flecha + pos
        posicion = pos + posicion
        pos = cuadro_afectaciones_tropas_comparativo(pdf, datos_res_afectaciones_ant, datos_res_afectaciones_act, "GAO-r", "GAO-RESIDUAL","GAO-r disidencias FARC","GAO - Residual Disidencias FARC",ruta, flecha)
        flecha = flecha + pos
        posicion = pos + posicion
        pos = cuadro_afectaciones_tropas_comparativo(pdf, datos_res_afectaciones_ant, datos_res_afectaciones_act, "GAO-CG", "GAO CLAN DEL GOLFO","GAO CLAN DEL GOLFO","GAO CLAN DEL GOLFO", ruta, flecha)
        flecha = flecha + pos
        posicion = pos + posicion
        pos = cuadro_afectaciones_tropas_comparativo(pdf, datos_res_afectaciones_ant, datos_res_afectaciones_act, "GAO-CAP", "GAO CAPARROS","GAO CAPARROS","GAO CAPARROS", ruta, flecha)
        flecha = flecha + pos
        posicion = pos + posicion    
        pos = cuadro_afectaciones_tropas_comparativo(pdf, datos_res_afectaciones_ant, datos_res_afectaciones_act,"GAO-PEL", "GAO PELUSOS","GAO PELUSOS","GAO PELUSOS", ruta, flecha)
        flecha = flecha + pos
        posicion = pos + posicion
        pos = cuadro_afectaciones_tropas_comparativo(pdf, datos_res_afectaciones_ant, datos_res_afectaciones_act, "GDO","GDO","GDO","GDO", ruta, flecha)
        flecha = flecha + pos
        posicion = pos + posicion
        pos = cuadro_afectaciones_tropas_comparativo(pdf, datos_res_afectaciones_ant, datos_res_afectaciones_act, "NARCO","NARCOTRÁFICO","NARCOTRÁFICO","NARCOTRÁFICO", ruta, flecha)
        flecha = flecha + pos
        posicion = pos + posicion
        pos = cuadro_afectaciones_tropas_comparativo(pdf, datos_res_afectaciones_ant, datos_res_afectaciones_act, "DELCO","DELINCUENCIA","DELINCUENCIA","DELINCUENCIA", ruta, flecha)
        flecha = flecha + pos
        posicion = pos + posicion
        pdf.set_font('Arial', 'B', 10)
        pdf.ln(5)
...
def grupo_result_resal_comparativa (pdf, pos_vert, pos, elemento, datos_res_ant, datos_res_act, suma, imagen, filtro, const):
    ...
    pdf.set_font('Arial', '', 10)
    pdf.set_text_color(0, 0, 0)
    pdf.image(direccion,pos_vert[0], pos-5,6,6)
...
def titulo_comparatibo_muñecos(pdf, pos_vert, pos, anio_ant, anio_act, text, filtro, largo ):
        ...
        pdf.set_line_width(0.5)
        pdf.line(pos_vert[0], pos-5, pos_vert[6], pos-5)
...
def res_afectaciones_sin_enemigo(pdf, datos, division, enemigo_evaluar, posicion_division, suma, evento, celdas):
    ...
    if datos_division:
        ...
        if evento == "Kg Cocaína" or evento == "Kg Marihuana" or evento == "Kg P.B.C":
            ...
            numero = formato_numero_decimal(numero)
...
def resultados_diarios_divisiones_coe(pdf, res_calculo_act, division, posiciones):
    ...
    #menores
    numero = llamado_afectacion(pdf, res_calculo_act[0], division, "DELINCUENCIA", 1, 18, "RME", numero)


    #PRESEMTACIONES 
    numero = llamado_afectacion(pdf, res_calculo_act[1], division, "DELINCUENCIA", 1, 18, "Pres. Volt.", numero)
    #SOMETETIMIENTOS
    numero = llamado_afectacion(pdf, res_calculo_act[2], division, "DELINCUENCIA", 1, 18, "Sometimientos.", numero)
    #CAPTURAS
    numero = llamado_afectacion(pdf, res_calculo_act[3], division, "DELINCUENCIA", 1, 18, "Captura", numero)
    #MDOM
    numero = llamado_afectacion(pdf, res_calculo_act[4], division, "DELINCUENCIA", 1, 18, "MDOM", numero)


    #ASESINADOS
    numero = llamado_afectacion(pdf, res_calculo_act[5], division, "DELINCUENCIA", 1, 18, "Asesinado", numero)
    #HERIDOS
    numero = llamado_afectacion(pdf, res_calculo_act[6], division, "DELINCUENCIA", 1, 18, "Herido", numero)
    ...
    #Cordón Detonante m
    numero = llamado_afectacion_sin_enemigo(pdf, res_calculo_act[19], division, "", 1, 18, "m Cordón Detonante m", numero)
    #Mecha Lenta m
    numero = llamado_afectacion_sin_enemigo(pdf, res_calculo_act[20], division, "", 1, 18, "m Mecha Lenta m", numero)
    ...
    #Dragas
    numero = llamado_afectacion_sin_enemigo(pdf, res_calculo_act[20], division, "", 1, 18, "Dragas", numero)
...
def claculo_afectacion_amenaza_cabecera(pdf, anio_act, anio_ant):
    pdf.ln(10)
    pdf.cell(30)
    pdf.set_fill_color(94, 119, 89)
    pdf.set_draw_color(0, 0, 0)
    ...
    lista_enemigo = ["GAO ELN","GAO-r","GAO CG","GDO","NARCO", "TOTAL"]

    for enemigo in lista_enemigo:
        if enemigo != "TOTAL":
            pdf.cell(51.5,10,str(enemigo),1,0, 'C',True)
        else:
    ...
    for enemigo in lista_enemigo:
        if enemigo != "TOTAL":
            pdf.cell(12.5,5,anio_ant,1,0, 'C',True)
            pdf.cell(12.5,5,anio_act,1,0, 'C',True)
            pdf.cell(12.5,5,"DIF.",1,0, 'C',True)
...
def celda_afectacion(pdf, dato_ant, dato_act, ruta, vert, altu ):
            ...
            pdf.cell(12.5,10,str(formato_numero(diferencia)),0,0, 'C',False)
...
def afectacion_ameneza_enemigo(pdf, afectacion, ln, ln2, cell, datos_res_ant, datos_res_act, suma, constante, id_enemigo, ruta, alt):
    ...
    lista_enemigo = ["GAO ELN","GAO-r","GAO CLAN DEL GOLFO","GDO","NARCOTRÁFICO", "TOTAL"]
    vert = 52.5
    lista_act = []
    lista_ant = []
    ...
    for enemigo in lista_enemigo:
        ...
        if enemigo != "GAO-r" and enemigo != "TOTAL":
            ...
            lista_act.append(total_act)
            lista_ant.append(total_ant)
            
    pdf.ln(ln)
    return [lista_ant, lista_act]


def afectacion_ameneza_enemigo_total(pdf, afectacion, ln, ln2, cell, datos_res_ant, datos_res_act, suma, constante, id_enemigo, ruta, alt):
        # pdf.text(5,num2+1, text)
    pdf.ln(ln)
    pdf.cell(cell)
    pdf.set_font('Arial', 'B', 10)
    ...
    for enemigo in lista_enemigo:
        ...
        TOTAL_ANT =datos_res_ant[numero]+datos_res_ant[numero+6]+datos_res_ant[numero+6+6]+datos_res_ant[numero+6+6+6]+datos_res_ant[numero+6+6+6+6]
        TOTAL_ACTU = datos_res_act[numero]+datos_res_act[numero+6]+datos_res_act[numero+6+6]+datos_res_act[numero+6+6+6]+datos_res_act[numero+6+6+6+6]
        numero = numero +1 

        celda_afectacion(pdf, TOTAL_ANT, TOTAL_ACTU, ruta, vert-20, alt )
