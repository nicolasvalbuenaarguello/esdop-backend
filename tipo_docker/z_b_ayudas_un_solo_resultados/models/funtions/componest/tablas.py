import math
import datetime
def encabezado_coe(pdf, fill, divisiones):
    pdf.set_line_width(0.1)
    pdf.set_fill_color(20, 62, 52)
    pdf.set_draw_color(0, 0, 0)
    pdf.set_text_color(255,255,255)
    pdf.set_font('Arial Narrow', 'B', 8.5)
    
    pdf.ln(-12)

    pdf.cell(130)
    
    pdf.cell(44,4,"DIVISIONES",1,0, 'C',fill)
    numero = 44
    for x in divisiones:
        pdf.cell(12,8,str(x),1,0, 'C',fill)
        numero=numero +12


    pdf.cell(14,8,"TOTAL",1,0, 'C',fill)
    numero = numero +14
    ...
    pdf.ln(4)
    pdf.cell(130)
    pdf.cell(44,4,"EVENTO",1,0, 'C',False)

    pdf.ln(4)

    return numero
def convencion(pdf, ruta, altura, anchura):
    ...
    pdf.rounded_rect(altura_0, anchura_0, 29, 35, 1,'DF', '1234')
    pdf.set_text_color(0,0,0)
    pdf.set_font('Arial', 'B', 8.5)
    pdf.text(altura,anchura,"CONCENTRACIÓN")
    anchura_2 = anchura +3.5
    pdf.text(altura,anchura_2,"DE EVENTOS")
    pdf.set_font('Arial', 'B', 7.5)
    altura = altura +6
    anchura_3 = anchura +8
    # pdf.text(10,42.5,fecha_titulo_tres)
    pdf.text(altura,anchura_3,"81%  A 100 %")
    anchura_3 = anchura_3 +5
    pdf.text(altura,anchura_3, "61%  A 80 %")
    anchura_3 = anchura_3 +5
    pdf.text(altura,anchura_3, "41%  A 60 %")
    anchura_3 = anchura_3 +5
    pdf.text(altura,anchura_3, "21%  A 40 %")
    anchura_3 = anchura_3 +5
    pdf.text(altura,anchura_3, "1%  A 20 %")
    altura = altura -5
    anchura_3 = anchura +6
    img = '{}static/img/img_mapas/100.png'.format(ruta) 
    pdf.image(img,altura, anchura_3, 3, 3)
    anchura_3 = anchura_3 +5
    img = '{}static/img/img_mapas/80.png'.format(ruta) 
    pdf.image(img,altura, anchura_3, 3, 3)
    anchura_3 = anchura_3 +5
    img = '{}static/img/img_mapas/60.png'.format(ruta) 
    pdf.image(img,altura, anchura_3, 3, 3)
    anchura_3 = anchura_3 +5
    img = '{}static/img/img_mapas/40.png'.format(ruta) 
    pdf.image(img,altura, anchura_3, 3, 3)
    anchura_3 = anchura_3 +5
    img = '{}static/img/img_mapas/20.png'.format(ruta) 
    pdf.image(img,altura, anchura_3, 3, 3)

def encabezado_coe_2(pdf, fill, divisiones):
    pdf.set_line_width(0.1)
    pdf.set_fill_color(20, 62, 52)
    pdf.set_draw_color(0, 0, 0)
    pdf.set_text_color(255,255,255)
    ...
    pdf.cell(26,4,"DIVISIONES",1,0, 'C',fill)
    numero = 26
    for x in divisiones:
        pdf.cell(13.75,8,str(x),1,0, 'C',fill)
        numero=numero +13.75
    ...
    pdf.ln(4)
    pdf.cell(125)
    pdf.cell(26,4,"EVENTO",1,0, 'C',False)
...
def separador_cuadro_coe_2(pdf, fill, text, numero):
    ...
    pdf.set_text_color(0,0,0)
    pdf.set_fill_color(227, 227, 211)
    pdf.cell(numero,4,text,1,0, 'C',fill)
    pdf.ln(4)
def separador_cuadro_coe(pdf, fill, text, numero):
...
def estadistica_resultados(datos_res, hechos, filtro):
    ...
    activacion_explosivos =[]
    activacion_zona_minada =[]

    acto_terrorismo_ =[]
    acto_terrorismo_infraestrucra =[]
    acto_terrorismo_poblacion =[]
    acto_terrorismo_tropas =[]

   # 'ACTO DE TERRORISMO', 'ACTO DE TERRORISMO EN CONTRA DE INFRAESTRUCTURA CRITICA', 'ACTO DE TERRORISMO EN CONTRA DE POBLACIÓN CIVIL', 'ACTO DE TERRORISMO EN CONTRA DE PROPIAS TROPAS'
    ...
    mecha = []
    medios_lanzamientos = []
    detonador_ini_electrico = []
    detonador_electrico = []
    artefactos_explosivos = []
    ...
    maquinaria_amarilla = []
    motores = []
    combustibles_mineria = []
    explosivos_mineria_kg = []
    explosivos_mineria_m = []
    explosivos_mineria_und = []
    material_transporte = []
    coltan = []
    ...
    capturas_contrabando =[]
    rme_contrabando =[]
    combustibles_contrabando =[]
    gasolina_contrabando =[]
    acpm_contrabando =[]
    vehiculos_contrabando =[]
    insumos_liquidos_contrabando =[]
    insumos_liquidos_contrabando =[]
    animales_incautados_contrabando =[]


    for x in nueva:
        if x == "MENORES RECUPERADOS":
            #MENORES RECUPERADOS
        ...
        if x == "SOMETIMIENTOS A LA JUSTICIA":
            #SOMETIMIENTOS
        ...
        if x == "PRESENTACIONES VOLUNTARIAS":
    ...
    # print(asesinados)
        ...
        if x == "ATAQUES DRONS": 
            ...
            ataque_con_drons = list(filter(lambda ataque_con_drons: "EJERCITO" in ataque_con_drons[12], ataque_con_drons))
        ...
        if x == "ARTEFACTOS EXPLOSIVOS Und": 
        ...
        if x == "MATAS DE COCA EN SEMILLEROS": 
            #matas coca
        ...
        if x == "PASTA DE COCA EN PROCESO": 
            #pbc en proceso
            tipo = list(filter(lambda datos_res: datos_res[10] == "NARCOTRAFICO" and datos_res[11] == "DROGA", datos_res))
            pbc_proceso = list(filter(lambda tipo: tipo[12] == "BASE DE COCA EN PROCESO" or tipo[12] == "PASTA DE COCA EN PROCESO",  tipo))
        
        if x == "CAPTURAS LOE AMAZONÍA": 
            #plan amazonia
            #capturas
            #res accion capturas
            res_capturas = list(filter(lambda datos_res: "CAPTURADO(A)" in datos_res[9], datos_res))
            capturas_artemisa = list(filter(lambda res_capturas: "Afectación Recursos Naturales y Medio Ambiente" in res_capturas[17], 
                                        ))
        if x == "PLÁNTULAS SEMBRADAS": 
            #plantulas sembradas
            plantulas_sembradas = list(filter(lambda datos_res: "SEMBRADO(S)" in datos_res[9], datos_res))
        if x == "INCAUTACIÓN MADERA M3": 
            #madera incautada
        ...
        if x == "ESPECIES ANIMALES INCAUTADOS": 
        ...
        if x == "AFECTACIONES AL OLEODUCTO": 
        ...
        if x == "DEPOSITOS ILEGALES": 
            #depositos
            depositos = list(filter(lambda datos_res: "INFRAESTRUCTURA" in datos_res[10], datos_res))
            depositos = list(filter(lambda depositos: "CRIMINAL" in depositos[11], depositos))
            depositos = list(filter(lambda depositos: "DEPÓSITO ILEGAL" in depositos[12], depositos)) 
        ...
        if x == "INSUMOS LIQUIDOS DE CONTRABANDO":
            estrategia_contrabando = list(filter(lambda datos_res:  datos_res[17] == "Contrabando" , datos_res))
            insumos_liquidos_contrabando = list(filter(lambda estrategia_contrabando:  "INSUMOS" in estrategia_contrabando[10] and estrategia_contrabando[13] == "Gal" , estrategia_contrabando))      
                    
        if x == "INSUMOS SOLIDOS DE CONTRABANDO":
            estrategia_contrabando = list(filter(lambda datos_res:  datos_res[17] == "Contrabando" , datos_res))
            insumos_liquidos_contrabando = list(filter(lambda estrategia_contrabando:  "INSUMOS" in estrategia_contrabando[10] and estrategia_contrabando[13] == "Kg" , estrategia_contrabando))  
            
        if x == "SEMOVIENTES DE COMTRABANDO":
            estrategia_contrabando = list(filter(lambda datos_res:  datos_res[17] == "Contrabando" , datos_res))
...
def mes(date):
        ...
        if date == "12" or date == 12:
            return months[11]
def meses_año_combate(pdf, datos_res, text, meses, anios):
    ...
    for x in meses:
        ...
        for dato in datos_res:
            format = '%Y-%m-%d'
            datetime_str = datetime.datetime.strptime(str(dato[1]), format)
            mes_n = str(datetime_str.strftime('%m'))
            año = str(datetime_str.strftime('%Y'))
            mes_d = str(mes(mes_n))
            if x == mes_d:
                valor_mes =  valor_mes + float(dato[14])

        resultados.append(valor_mes)
    ...
    for x in anios:
            ...
            resultados_anio.append(valor_mes)


    return [resultados, resultados_anio]
    
#funcion que dibuja las divisiones en cuadro del  boletin del coe 
def meses_año(pdf, datos_res, text, meses, anios):
    ...
    for x in meses:
        ...
        for dato in datos_res:
            format = '%Y-%m-%d'
            datetime_str = datetime.datetime.strptime(str(dato[0]), format)
            mes_n = str(datetime_str.strftime('%m'))
            año = str(datetime_str.strftime('%Y'))
            mes_d = str(mes(mes_n))
            if x == mes_d:
                valor_mes =  valor_mes + float(dato[18])
    ...
    return [resultados, resultados_anio]
#funcion que dibuja las divisiones en cuadro del  boletin del coe 
def sin_abla_boletin_coe_combate(pdf, datos_res, text, divisiones, indicador):
    numero = calculo_total(datos_res, indicador, 14)
    # numero = 0
    if numero != "-":
        ...
        for x in divisiones:
            ...
            numero = calculo(datos_res, str(x), indicador, 14)
...
#funcion que dibuja las divisiones en cuadro del  boletin del coe 
def sin_abla_boletin_coe(pdf, datos_res, text, divisiones, indicador):
    numero = calculo_total(datos_res, indicador, 18)
    # numero = 0
    resultados = []
    if numero != "-":
        ...
        for x in divisiones:
            ...
            numero = calculo(datos_res, str(x), indicador, 18)
            resultados.append(numero)
...
#funcion que dibuja las divisiones en cuadro del  boletin del coe 
def tabla_boletin_coe_n(pdf, datos_res, text, divisiones, indicador):
    resultados = []  # <--- Siempre definida
    ...
    if numero != "-":
        ...
        pdf.cell(44, 4, text, 1, 0, 'L', True)

        numero_f = 44
        for x in divisiones:
            numero = calculo(datos_res, str(x), indicador, 18)
            resultados.append(numero)
            pdf.cell(12, 4, str(formato_numero(numero)), 1, 0, 'C', False)
            numero_f += 12

        pdf.set_fill_color(125, 0, 0)
        pdf.set_text_color(255, 255, 255)
        numero = calculo_total(datos_res, indicador, 18)
        pdf.cell(14, 4, numero, 1, 0, 'C', True)
        numero_f += 14
        pdf.ln(4)

    return resultados
def tabla_boletin_coe(pdf, datos_res, text, divisiones, indicador):
    resultados = []  # <--- Esto evita el UnboundLocalError
...
def tabla_boletin_coe_2(pdf, datos_res, text, divisiones):
    ...
    if numero != "-":
        ...
        pdf.cell(125)
        pdf.set_font('Calibri', 'B', 6)
        pdf.cell(26,4,text,1,0, 'L',True)
        ...
        for x in divisiones:
            ...
            numero = calculo(datos_res, str(x), 1, 18)
            pdf.cell(13.75,4,numero,1,0, 'C',False)
            numero_f = numero_f  + 12
...
def tabla_boletin_coe_hechos(pdf, datos_res, text, divisiones):
    ...
    if numero !="-":
        ...
        for x in divisiones:
            ...
            total = 0
            numero = calculo(datos_res, str(x), 2, 14)
            resultados.append(numero)
            pdf.cell(12,4,str(numero),1,0, 'C',False)

        pdf.set_fill_color(125, 0, 0)
        pdf.set_text_color(255,255,255)
        numero = calculo_total(datos_res, 2, 14)
        pdf.cell(13,4,numero,1,0, 'C',True)
...
# def encabezado_comparativo_mapa(pdf, fill, anio_act, anio_ant, anio_ant_tres, ancho):
#     pdf.ln(6)
#     pdf.set_line_width(0.1)
#     pdf.set_fill_color(20, 62, 52)
#     pdf.set_draw_color(0, 0, 0)
...
#     porc = "% "+str(anio_ant_tres)+"   / "+str(anio_ant)

#     pdf.multi_cell(34, 8, "ITEM", 1, "C", fill)
#     pdf.ln(-8)
#     pdf.cell(ancho+34)
#     pdf.multi_cell(17, 8, str(anio_ant_tres), 1, "C", fill)
#     pdf.ln(-8)
#     pdf.cell(ancho+34+17)
#     pdf.multi_cell(17, 8, str(anio_ant), 1, "C", fill)
#     pdf.ln(-8)
#     pdf.cell(ancho+34+17+17)
#     pdf.multi_cell(15, 4, str(porc), 1, "C", fill)
#     pdf.ln(-8)
#     pdf.cell(ancho+34+17+17+15)

#     pdf.multi_cell(17, 8, str(anio_act), 1, "C", fill)
#     pdf.ln(-8)
#     pdf.cell(ancho+34+17+17+15+17)
#     porc = "% "+str(anio_ant)+"   / "+str(anio_act)
#     pdf.multi_cell(15, 4, str(porc), 1, "C", fill)
...
# def sepador_mapa(pdf, fill, text, cell, ln, acho):
...
#     pdf.ln(ln)
#     pdf.cell(cell)
#     pdf.cell(acho,5,text,1,0, 'C',fill)
...
def sepador_mapa_dos(pdf, fill, text, cell):
    ...
    pdf.cell(165,5,text,1,0, 'C',fill)
    pdf.set_fill_color(240,240,240)
def claculo_mapa_mapa(pdf, datos_res_ant,  datos_res_act, res_calculo_ante_tres, text, constante, id_enemigo, suma, ancho, ruta, altu_img, pos_img):
    # pdf.ln(5)

    maquinari_amarialla_anterior= 0
    maquinari_amarialla_actual= 0
    maquinari_amarialla_anterior_tres= 0
    res = 0
    ...
    res = 0
    res = calculo_total_sin_filtro_b(res_calculo_ante_tres[0], suma)
    maquinari_amarialla_anterior_tres = maquinari_amarialla_anterior_tres + res
    res = calculo_total_sin_filtro_b(res_calculo_ante_tres[1], suma)
    maquinari_amarialla_anterior_tres = maquinari_amarialla_anterior_tres + res
    res = calculo_total_sin_filtro_b(res_calculo_ante_tres[2], suma)
    maquinari_amarialla_anterior_tres = maquinari_amarialla_anterior_tres + res
    res = calculo_total_sin_filtro_b(res_calculo_ante_tres[3], suma)
    maquinari_amarialla_anterior_tres = maquinari_amarialla_anterior_tres + res
    res = calculo_total_sin_filtro_b(res_calculo_ante_tres[4], suma)
    maquinari_amarialla_anterior_tres = maquinari_amarialla_anterior_tres + res


    # resultados  = calculo_comprativo_total(maquinari_amarialla_anterior, maquinari_amarialla_actual, suma, constante)
    resultados_por = porcentajes(maquinari_amarialla_anterior_tres, maquinari_amarialla_anterior, constante)

    maquinari_amarialla_anterior_tres = formato_numero(maquinari_amarialla_anterior_tres)
    maquinari_amarialla_anterior_2 = formato_numero(maquinari_amarialla_anterior)
    ...
    pdf.multi_cell(34, 4, text, 1, "L", False)
    final = pdf.get_y()
    x = posicion_inicial-final

    # pdf.ln(x)
    pdf.cell(ancho+34)

    pdf.multi_cell(17, x, maquinari_amarialla_anterior_tres, 1, "C", False)
    pdf.ln(x*-1)
    pdf.cell(ancho+34+17)
    pdf.multi_cell(17, x, maquinari_amarialla_anterior_2, 1, "C", False)
    pdf.ln(x*-1)
    pdf.cell(ancho+34+17+17)

    
    if constante == 1:
        resultados = formato_numero(resultados_por)
        if resultados_por > 0:
            pdf.set_text_color(0,128,0)
            triangulo_1 = '{}static/img/tri_verde_new.png'.format(ruta)

        elif resultados_por < 0:
            pdf.set_text_color(128,0,0)
            triangulo_1 = '{}static/img/tri_rojo_new.png'.format(ruta)

        else:
            pdf.set_text_color(232,172,2)
            triangulo_1 = '{}static/img/tri_amarillo_new.png'.format(ruta)
    else:
        if resultados_por < 0:
            pdf.set_text_color(128,0,0)
            triangulo_1 = '{}static/img/tri_rojo_new_pos_con.png'.format(ruta)

        elif resultados_por > 0:
            pdf.set_text_color(0,128,0)
            triangulo_1 = '{}static/img/tri_verde_new_neg_con.png'.format(ruta)
    ...
    pdf.multi_cell(15, x, resultados, 1, "R", False)
    pdf.set_text_color(0,0,0)



    resultados_por = porcentajes(maquinari_amarialla_anterior, maquinari_amarialla_actual, constante)
    ...
    pdf.ln(x*-1)
    pdf.cell(ancho+34+17+17+15)
    pdf.multi_cell(17, x, maquinari_amarialla_actual, 1, "C", False)
    pdf.ln(x*-1)
    pdf.cell(ancho+34+17+17+15+17)
    

    if constante == 1:
        porcentaje = formato_numero(resultados_por)
        if resultados_por > 0:
            pdf.set_text_color(0,128,0)
        ...
        if resultados_por < 0:
            ...
            pdf.set_text_color(232,172,2)
            triangulo = '{}static/img/tri_amarillo_new.png'.format(ruta)
        porcentaje = formato_numero(resultados[2])
    
    pdf.multi_cell(15, x, porcentaje, 1, "R", False)
    pdf.ln(x*-1)
    pdf.set_text_color(0,0,0)
    ...
    if x > 4:
        ...
        pos_img =pos_img + x + 1.8
    pdf.image(triangulo_1,altu_img,pos_img,2.25,2.25)#imagenes
    altu_img=altu_img+32
    pdf.image(triangulo,altu_img,pos_img,2.25,2.25)#imagenes
    if x <= 4:
        pos_img = pos_img -1.8
    return pos_img
...
# def claculo_mapa(pdf, datos_res_ant,  datos_res_act, res_calculo_ante_tres, text, constante, id_enemigo, suma, ancho, ruta, altu_img, pos_img):
#     # pdf.ln(5)
#     pdf.cell(ancho)
#     pdf.set_text_color(0,0,0)
...
#     # pdf.ln(x)
#     pdf.cell(ancho+34)
#     resultados  = calculo_comprativo_total(res_calculo_ante_tres, datos_res_ant, suma, constante)
#     pdf.multi_cell(17, x, resultados[0], 1, "C", False)
#     pdf.ln(x*-1)
#     pdf.cell(ancho+34+17)
#     pdf.multi_cell(17, x, resultados[1], 1, "C", False)
#     pdf.ln(x*-1)
#     pdf.cell(ancho+34+17+17)
...
def cuadro_afectaciones(pdf, datos, filtro):
    ...
    asesinados = '{}scr/static/img/img_mapas/asesinados.jpg'.format(ruta)
    heridos = '{}scr/static/img/img_mapas/heridos.jpg'.format(ruta)
    soldados = '{}scr/static/img/img_mapas/soldados.jpg'.format(ruta)
...
def grupo_result_resal(pdf, pos_vert, pos, elemento, datos_res, suma, imagen, filtro):
    ...
    ruta = '{}scr/static/img/resultados/'.format(filtro[15])
...
def calculo_boletin_afectaciones_total(pdf, titulo, ln, datos_res_ant, datos_res_act,  constante, posicion, filtro, posicionamiento):
    ...
    ruta_b='{}scr/static/img/{}.png'.format(filtro[15],imagen)
...
def titulo_boletin_dirop(pdf, filtro):
    ...
    ruta_b='{}scr/static/img/cabecera_boletin_dirop.png'.format(filtro[15])

    pdf.image(ruta_b,0,0,220,30)#imagenes 

    ruta_b='{}scr/static/img/jemop.png'.format(filtro[15])
    pdf.image(ruta_b,205,0.2,10,10)#imagenes 

    ruta_b='{}scr/static/img/cuadro.png'.format(filtro[15])
    pdf.image(ruta_b,62,35.5,13,4.5)#imagenes 

    ruta_b='{}scr/static/img/cuadro_dos.png'.format(filtro[15])
...
def funcion_celda_comparativa(pdf,dato_ant, dato_act, ruta, flecha, posi, celda, conts, fill):
        ...
        if dato_por > 0:
            pdf.set_text_color(0,128,0)
            triangulo = '{}scr/static/img/tri_verde_new.png'.format(ruta)

        elif dato_por < 0:
            pdf.set_text_color(128,0,0)
            triangulo = '{}scr/static/img/tri_rojo_new.png'.format(ruta)

        else:
            pdf.set_text_color(232,172,2)
            triangulo = '{}scr/static/img/tri_amarillo_new.png'.format(ruta)
...
def grupo_result_resal_comparativa (pdf, pos_vert, pos, elemento, datos_res_ant, datos_res_act, suma, imagen, filtro, const):
    ...
    if dato_por > 0:
        pdf.set_text_color(0,128,0)
        triangulo = '{}scr/static/img/tri_verde_new.png'.format(filtro[15])

    elif dato_por < 0:
        pdf.set_text_color(128,0,0)
        triangulo = '{}scr/static/img/tri_rojo_new.png'.format(filtro[15])

    else:
        pdf.set_text_color(232,172,2)
        triangulo = '{}scr/static/img/tri_amarillo_new.png'.format(filtro[15])
