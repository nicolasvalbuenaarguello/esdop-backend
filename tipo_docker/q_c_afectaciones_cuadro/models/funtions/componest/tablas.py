...
def estadistica_resultados(datos_res, hechos, filtro):
    ...
    asesinados = list(filter(lambda asesinados: asesinados[11] != "CIVILES", asesinados))
    ...
    heridos = list(filter(lambda heridos: heridos[11] != "CIVILES",  heridos))
...
def suma_h_a(resultado_asesinados, resultado_heridos ):
    if resultado_asesinados == "-":
        ...
        resultado_asesinados = str(resultado_asesinados).replace(".","")
        res_asesinado = resultado_asesinados

    if resultado_heridos == "-":
        res_herido = 0
    else:
        resultado_heridos = str(resultado_heridos).replace(".","")
...
def cuadros_heridos_asesinados(pdf, asesinados, heridos):
    ...
    res_asesinados = calculo(asesinados, "SOLDADOS", 11, 18)
    res_heridos = calculo(heridos, "SOLDADOS", 11, 18)
    
    total = suma_h_a(res_asesinados, res_heridos)

    text = (x, y, "3","SOLDADOS ", str(res_asesinados), str(res_heridos), str(total), a, fill )
    celda_afectaciones(pdf, 5, 5, text, rango, id)
    ...
    #-------------------------------------
    res_asesinados = calculo(asesinados, "BACRIM", 7, 18)
    res_heridos = calculo(heridos, "BACRIM", 7, 18)
    total = suma_h_a(res_asesinados, res_heridos)

    if total != "-":
        numero = numero +1
        text = (x, y, str(numero),"BACRIM", str(res_asesinados), str(res_heridos), str(total), a, fill )
        celda_afectaciones(pdf, 5, 172, text, rango, id)
    
    res_asesinados = calculo(asesinados, "EPL", 7, 18)
    res_heridos = calculo(heridos, "EPL", 7, 18)
    total = suma_h_a(res_asesinados, res_heridos)

    if total != "-":
        numero = numero +1
        text = (x, y, str(numero),"EPL", str(res_asesinados), str(res_heridos), str(total), a, fill )
        celda_afectaciones(pdf, 5, 172, text, rango, id)
        
    res_asesinados = calculo(asesinados, "FARC", 7, 18)
    res_heridos = calculo(heridos, "FARC", 7, 18)
    total = suma_h_a(res_asesinados, res_heridos)

    if total != "-":
        numero = numero +1
        text = (x, y, str(numero),"FARC", str(res_asesinados), str(res_heridos), str(total), a, fill )
        celda_afectaciones(pdf, 5, 172, text, rango, id)
    
          
    res_asesinados = calculo(asesinados, "NO APLICA", 7, 18)
    res_heridos = calculo(heridos, "NO APLICA", 7, 18)
    total = suma_h_a(res_asesinados, res_heridos)

    if total != "-":
        numero = numero +1
        text = (x, y, str(numero),"NO APLICA", str(res_asesinados), str(res_heridos), str(total), a, fill )
        celda_afectaciones(pdf, 5, 172, text, rango, id)
...
def variables_acciones(pdf, datos, cell, posicion_inicial_final, texto_ingresado):
    acciones_asecinados  =[]
    ...
    pdf.set_draw_color(0, 0, 0)
    pdf.set_text_color(255,255,255)
    texto = "{} POR ACCIÓN DIRECTA DEL ENEMIGO".format(texto_ingresado)
    pdf.cell(163.5,7,texto,1,0, 'C',True)
    pdf.set_fill_color(208,208,182)
    pdf.set_text_color(0,0,0)
    pdf.set_font('Arial', 'B', 12)
