from flask import make_response
from datetime import date, time, datetime

from tipo_docker.q_c_afectaciones_cuadro.models.estadistica.estadistica_boletin_coe import *
from __init__ import *
from tipo_docker.z_z_configuarcion.caligrafia import *
from tipo_docker.z_z_configuarcion.fechas import *
from tipo_docker.z_z_configuarcion.header import *
from tipo_docker.z_z_configuarcion.logo import *
from tipo_docker.z_z_configuarcion.titulos import *  
#Funcion para la creacion del reporte de resultados 


#Función en Python que genera el cuadro de las afectaciones de propias tropas 
def afectaciones_cuadro(fecha_inicial_u_l, fecha_final_u_l, filtro, dirercion_archvios, nombre_carpeta):





    pdf = PDF(orientation = 'L', unit = 'mm', format='legal')

    caligrafia_ingreso( pdf, filtro[15])

    fechas_inicial = fecha(fecha_inicial_u_l)
    fechas_final = fecha(fecha_final_u_l)

    fecha_titulo = fechas_inicial[0]+" " +  fechas_inicial[1] + " AL " + fechas_final[0] + " "+fechas_final[1] + " "+fechas_final[2]
    fecha_titulo = fecha_titulo.upper()
    
    titulo_unidad  =  titulos_name_afectaciones(filtro, fechas_final[2])

    if len(titulo_unidad[1])>0:
        municipios=tuple(titulo_unidad[1])
    else: 
        municipios =""
    titulo= titulo_unidad[0]

    img_qr =  "C:/Users/nicolas.valbuena/Documents/programacion 2023/server/tipo_docker/q_c_afectaciones_cuadro/models/qr_img"
    fecha_titulo_sub = 'PROTEGER Y FORTALECER LA FUERZA Y SUS CAPACIDADES ESTRATÉGICAS'
    pdf.parametros(titulo = titulo, tamanio = "oficio", logo = filtro[11], fecha_titulo = fecha_titulo_sub, permiso =filtro[12], nivel = filtro[13], usuario = filtro[14], pie_pagina = "SI" , ruta = filtro[15],  imagen = "static/img/oficio/Diapositiva18_DISEO.JPG", seguridad="nueva_sin_mapa", img_qr = img_qr, fecha_titulo_2 = fecha_titulo)
    # pdf.set_margin(10,10,10,True)
    pdf.set_auto_page_break(True, 6)
    pdf.add_page()
        
    if municipios !="":
        pdf.set_text_color(70,70,70)
        if len(municipios) <= 110:
            pdf.set_font('BebasNeue', '', 14)
        else:
            pdf.set_font('BebasNeue', '', 12)
        municipios =str(municipios)
        municipios.replace("[", "")
        municipios.replace("]", "")
        municipios.replace(",", " ")
        pdf.text(55,34,str(municipios))
#--------

            # print(titulo)
    if  filtro[4] == "lugar":
        if filtro[6]!="" and filtro[6]!="---" :#filtro por municipio
            municipios = str(filtro[6])
        else:
            titulo = titulo
    else:
        if titulo != "":
            titulo = titulo
        else:
            titulo = "RESULTADO RESALTANTES DEL EJÉRCITO NACIONAL"
            nombre_doc = titulo
    
    # fechas en el titulo
    resultados_spoa = Calculo_Spoa()
    resultados_spoa.cuadros_afectaciones_pdf(pdf, fecha_inicial_u_l, fecha_final_u_l, filtro)

    direcion = dirercion_archvios+str("afectaciones")+'.pdf'
    pdf.output(direcion, 'F')
    DIRECION_3 = os.getenv('DIRECION_3')
    direcion= DIRECION_3+nombre_carpeta+str("afectaciones")+'.pdf'
    return [direcion, "afectaciones"]

  


