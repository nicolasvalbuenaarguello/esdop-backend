...
from tipo_docker.b_f_resultados_lineas_estrategicas.models.estadistica.estadistica_boletin_coe import *
...
def comparativo_comparativo_mapa(fecha_inicial_u_l, fecha_final_u_l, fecha_inicial_p_l, fecha_final_p_l, filtro,  link, puerto):
    ...
    pdf.text(170,81, "EVALUACIÓN")
    pdf.text(170,91, "OBJETIVO")
    pdf.text(170,101, "ESTRATÉGICO #2")
    pdf.text(170,111, "“DEBILITAR LAS")
    pdf.text(170,121, "CAPACIDADES DE LA")
    pdf.text(170,131, "AMENAZA”")


    if filtro[0] == "DAVAA" :
        pdf.text(170,146, str("División de Aviación").upper())
        pdf.text(170,156, str("de Asalto Aéreo").upper())
    elif filtro[0] == "DIVFE":
        pdf.text(170,146, str("División de").upper())
        pdf.text(170,156, str("Fuerzas Especiales").upper())
    else:
        pdf.text(170,146, str(unidad))
    ...
    nombre_carpeta = f"resulotados_bj2_{puerto}/"
