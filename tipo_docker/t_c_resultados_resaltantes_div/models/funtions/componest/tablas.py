...
def estadistica_resultados(datos_res, hechos):
    ...
    res_capturas = list(filter(lambda res_capturas: "RME" not in res_capturas[9] , res_capturas))
    #capturas 
    ...
    #asesinados
    asesinados = list(filter(lambda datos_res: "ASESINADO(A)" in datos_res[9], datos_res))
    asesinados = list(filter(lambda asesinados: asesinados[7] != "NO APLICA", asesinados))
    asesinados = list(filter(lambda asesinados: "EJERCITO" in asesinados[12], asesinados))

    #heridos 
    heridos = list(filter(lambda datos_res: "HERIDO(A)" in datos_res[9], datos_res))
    heridos = list(filter(lambda heridos: heridos[7] != "NO APLICA", heridos))
    ...
    explosivos = list(filter(lambda explosivos: explosivos[13] == 'Kg', explosivos))
    ...
    #matas coca
    matas_coca = list(filter(lambda datos_res: "MATA(S) DE COCA EN SEMILLERO" in datos_res[12] , datos_res))
    
    #insumos liquidos
    afectacion_narcotrafico = list(filter(lambda datos_res: "Narcotráfico" in datos_res[17], datos_res))

    # insumos = list(filter(lambda afectacion_narcotrafico: "INSUMOS" in afectacion_narcotrafico[10] or "COMBUSTIBLES" in afectacion_narcotrafico[10], afectacion_narcotrafico))

    
    insumos = list(filter(lambda afectacion_narcotrafico: "AGROQUIMICOS" == afectacion_narcotrafico[11] or "INSUMOS LIQUIDOS" == afectacion_narcotrafico[11] or "LIQUIDOS" == afectacion_narcotrafico[11], afectacion_narcotrafico))
    
    insumos_liquidos = list(filter(lambda insumos:  insumos[13] == "Gal", insumos))
    # print(insumos_liquidos)

    #insumos solidos

    # insumos_solidos = list(filter(lambda afectacion_narcotrafico: "INSUMOS" in afectacion_narcotrafico[10] or "COMBUSTIBLES" in afectacion_narcotrafico[10], afectacion_narcotrafico))

    insumos = list(filter(lambda afectacion_narcotrafico: "AGROQUIMICOS" == afectacion_narcotrafico[11]  or "INSUMOS SOLIDOS" == afectacion_narcotrafico[11] or "SOLIDOS" == afectacion_narcotrafico[11], afectacion_narcotrafico))
    ...
    #maquinaria pesada
    maquinaria_pesada = list(filter(lambda afectacion_mineria: 'MAQUINARIA PESADA' in afectacion_mineria[12] , afectacion_mineria))
    
    #buldicer
    buldocer = list(filter(lambda afectacion_mineria: 'BULDOCER(ES)' in afectacion_mineria[12] , afectacion_mineria))
    #upm

    upm = list(filter(lambda afectacion_mineria: 'UPM ILEGAL' in afectacion_mineria[12] or 'UPM LEGAL' in afectacion_mineria[12] , afectacion_mineria))
    ...
    #motores
    motores = list(filter(lambda afectacion_mineria: 'MOTORES' in afectacion_mineria[12], afectacion_mineria))
    #Combustibles
    combustibles = list(filter(lambda afectacion_mineria: 'COMBUSTIBLES' in afectacion_mineria[10], afectacion_mineria))
    ...
    #Combustibles
    combustibles_incautados = list(filter(lambda datos_res: 'COMBUSTIBLES' in datos_res[10], datos_res))
    combustibles_incautados = list(filter(lambda combustibles_incautados:  combustibles_incautados[13] == "Gal", combustibles_incautados))
...
def resultados_diarios_divisiones_coe(pdf, res_calculo_act, division, posiciones):
    ...
    #Combates
    numero = llamado_afectacion_sin_enemigo(pdf, res_calculo_act[14], division, "", 2, 14, "Total de Combates", numero)
