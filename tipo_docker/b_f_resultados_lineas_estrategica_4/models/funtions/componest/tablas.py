...
def tabla_comparativa_linea_div(pdf, datos, datos_ant, id_enemigo, amenaza, suma, constante, cel, fill, color, indicadores):

    datos_res = calculo_comprativo_total_enemigo(datos, datos_ant, id_enemigo, amenaza, amenaza, suma, constante)
    pdf.cell(cel+15)
    pdf.cell(15,5,datos_res[0],1,0, 'C',fill)
    pdf.cell(15,5,datos_res[1],1,0, 'C',fill)
    pdf.cell(25,5,datos_res[3],1,0, 'C',fill)
    pdf.cell(25,5,datos_res[2],1,0, 'C',fill)
    if datos_res[4] >0:
        pdf.set_fill_color(125,0,0)
...
def tabla_comparativa_linea_bucle_total(pdf, datos, datos_ant, suma, constante, cel, fill, color, amenaza, id_enemigo):
    ...
    pdf.set_fill_color(color[0],color[1],color[2])
    pdf.set_text_color(0,0,0)
    return[datos_res[5],datos_res[6], datos_res[4], datos_res[2]]
...
def encabezado_comparativo_enemigo_dinamico_1(pdf, fill, datos, datos_ant, amenaza, cel, color, suma, constante, id_enemigo, indicadores):
    pdf.set_line_width(0.1)
    pdf.set_fill_color(color[0],color[1],color[2])
    pdf.set_draw_color(0, 0, 0)
    pdf.set_text_color(0,0,0)
    ...
    numero = 0
    for x in amenaza:
        indicadores = tabla_comparativa_linea_div(pdf, datos, datos_ant, id_enemigo, amenaza[numero], suma[numero], constante[numero], cel, fill, color, indicadores)


        pdf.ln(-5)
        pdf.cell(cel)
        pdf.cell(15,5,str(amenaza[numero]),1,0, 'C',fill)
        pdf.ln()
        numero =  numero +1

    resultados_total_afectaciones = tabla_comparativa_linea_bucle_total(pdf, datos, datos_ant, 18, 1, cel, fill,  color, amenaza,id_enemigo)
    pdf.ln(-5)
    pdf.cell(cel)
    pdf.cell(15,5,str("TOTAL"),1,0, 'C',fill)
    pdf.ln()

    return [resultados_total_afectaciones[0], resultados_total_afectaciones[1], resultados_total_afectaciones[2], resultados_total_afectaciones[3]]
