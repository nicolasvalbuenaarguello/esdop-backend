...
def estadistica_resultados(datos_res, hechos, filtro):
    ...
    capturas_miembros_fuerza = list(filter(lambda res_capturas: "CAPTURADO(A)" in res_capturas[9]  and res_capturas[7] == "NO APLICA", res_capturas)) 
    #capturas 
...
def tabla_boletin_coe(datos_res, text, divisiones, amenaza_lis, id_unidad):

    dato_evaluado=[]
    dato_evaluado.append(text)
    numero_index = 0
    for x in divisiones:
        numero = calculo(datos_res, x, id_unidad, 18)
        dato_evaluado.append(numero)


    numero = calculo_total(datos_res, 1, 18)
    dato_evaluado.append(numero)

    for x in amenaza_lis:
        numero = calculo(datos_res, x, 7, 18) 
        dato_evaluado.append(numero) 

    return dato_evaluado
...
def tabla_boletin_coe_hechos(datos_res, text,  divisiones,  amenaza_lis, id_unidad):
    dato_evaluado=[]
    dato_evaluado.append(text)
    for x in divisiones:
        numero = calculo(datos_res, x, id_unidad, 14)
        dato_evaluado.append(numero)

    
    numero = calculo_total(datos_res, 1, 14)
    dato_evaluado.append(numero)

    for x in amenaza_lis:
        numero = calculo(datos_res, x, 8, 14)
        dato_evaluado.append(numero)  
