...
#calculo comparativo
def calculo_comprativo_total_bucle(datos_res_ant, datos_res_act, identificador_suma, constante, amenaza, id_enemigo):
    numero_ant=0 
    numero_act=0
    for x in amenaza:
        for dato in datos_res_ant:
                if x == dato[id_enemigo]:
                    numero_ant =  numero_ant + float(dato[identificador_suma])
    ...
    porcentaje = porcentajes(numero_ant, numero_act, constante)
    numero_ant_a = formato_numero(numero_ant)
    numero_act_a = formato_numero(numero_act)
    porcentaje = formato_numero(porcentaje)
    diferencia_v = formato_numero(diferencia)
    
    return [numero_ant_a, numero_act_a, porcentaje, diferencia_v,  diferencia, numero_ant, numero_act]
...
def calculo_comprativo_total(datos_res_ant, datos_res_act, identificador_suma, constante):
    ...
    return [numero_ant, numero_act, porcentaje, diferencia_v,  diferencia]
...
def calculo_comprativo_total_enemigo(datos_res_ant, datos_res_act, id_parametro, parametro, parametro_dos, identificador_suma, constante):
    ...
    diferencia = (numero_act - numero_ant )*constante
    porcentaje = porcentajes(numero_ant, numero_act, constante)
    numero_ant_d = formato_numero(numero_ant)
    numero_act_d = formato_numero(numero_act)
    porcentaje = formato_numero(porcentaje)
    porcentaje_V = formato_numero(diferencia)
    
    return [numero_ant_d, numero_act_d, porcentaje, porcentaje_V, diferencia]
...
def tabla_comparativa_linea(pdf, datos, datos_ant, id_enemigo, amenaza, suma, constante, cel, fill, texto, color, indicadores):
    ...
    pdf.cell(25,5,datos_res[2],1,0, 'C',fill)
    if datos_res[4] < 0:
...
def tabla_comparativa_linea_div(datos, datos_ant, id_enemigo, amenaza, suma, constante, cel, fill, color, indicadores):

    datos_res = calculo_comprativo_total_enemigo(datos, datos_ant, id_enemigo, amenaza, amenaza, suma, constante)
    if int(datos_res[4]) >0:
        igual = "-"
    elif int(datos_res[4]) < 0:
        igual = "+"
    else:
        igual = "="
    dato = [amenaza, datos_res[0],datos_res[1],datos_res[3],datos_res[2], igual]

    return dato

def tabla_comparativa_linea_total(pdf, datos, datos_ant, id_enemigo, amenaza, suma, constante, cel, fill, texto, color, indicadores):
...
def tabla_comparativa_linea_2(pdf, datos, datos_ant, suma, constante, cel, fill, texto, color, amenaza):

    
    datos_res = calculo_comprativo_total_bucle(datos, datos_ant,  suma, constante, amenaza)
...
def tabla_comparativa_linea_bucle_total( datos, datos_ant, suma, constante, cel, fill, color, amenaza, id_enemigo):

    datos_res = calculo_comprativo_total_bucle(datos, datos_ant,  suma, constante, amenaza, id_enemigo)
    if int(datos_res[4])>0:
        igual = "-"
    elif int(datos_res[4])<0:
        igual = "+"
        ...
        igual = "="
    dato = ["TOTAL", datos_res[0],datos_res[1], datos_res[3], datos_res[2], igual]
    dato_2 =  [datos_res[5], datos_res[6], datos_res[2]]
    return[dato, dato_2]
...
def encabezado_comparativo_mapa_dos(pdf, fill, anio_act, anio_ant, cel, texto):
    pdf.set_line_width(0.1)
    ...
    pdf.cell(cel)
    pdf.cell(125,5,str(texto),1,0, 'C',fill)
    pdf.set_fill_color(255,255,255)
    pdf.set_text_color(0,0,0)
    pdf.set_font('Arial', 'B', 9)
    pdf.ln()
    pdf.cell(cel)
    pdf.cell(15,5,"UNIDAD",1,0, 'C',fill)
    pdf.cell(15,5,str(anio_ant),1,0, 'C',fill)
    pdf.cell(15,5,str(anio_act),1,0, 'C',fill)
    pdf.cell(25,5,"DIFERENCIA",1,0, 'C',fill)
    pdf.cell(25,5,"PORCENTAJE",1,0, 'C',fill)
...
def encabezado_comparativo_enemigo_dinamico(pdf, fill, datos, datos_ant, amenaza, cel, color, suma, constante, id_enemigo, texto, indicadores):
    pdf.set_line_width(0.1)
    pdf.set_fill_color(color[0],color[1],color[2])
    pdf.set_draw_color(0, 0, 0)
    pdf.set_text_color(0,0,0)
    ...
    numero = 0
    for x in amenaza:
        indicadores = tabla_comparativa_linea(pdf, datos[1], datos_ant[1], id_enemigo, amenaza[numero], suma[numero], constante[numero], cel, fill,texto[0] , color, indicadores)
        indicadores = tabla_comparativa_linea(pdf, datos[0], datos_ant[0], id_enemigo, amenaza[numero], suma[numero], constante[numero], cel, fill,texto[1] , color, indicadores)
        
        pdf.ln(-10)
        pdf.cell(cel)
        pdf.cell(20,10,str(amenaza[numero]),1,0, 'C',fill)
        pdf.ln()
        numero =  numero +1
...
def encabezado_comparativo_enemigo_dinamico_1(fill, datos, datos_ant, amenaza, cel, color, suma, constante, id_enemigo, indicadores):


    numero = 0
    valores=[]
    for x in amenaza:
        indicadores = tabla_comparativa_linea_div(datos, datos_ant, id_enemigo, amenaza[numero], suma[numero], constante[numero], cel, fill, color, indicadores)
        valores.append(indicadores)
        numero =  numero +1



    resultados_total_afectaciones = tabla_comparativa_linea_bucle_total( datos, datos_ant, 18, 1, cel, fill,  color, amenaza,id_enemigo)

    valores.append(resultados_total_afectaciones[0])

    return [valores, resultados_total_afectaciones[1]]


    
def encabezado_comparativo_enemigo_dinamico_TOTAL(pdf, fill, datos, datos_ant, amenaza, cel, color, suma, constante, id_enemigo, texto, indicadores):
    pdf.set_line_width(0.1)
    pdf.set_fill_color(color[0],color[1],color[2])
    pdf.set_draw_color(0, 0, 0)
    pdf.set_text_color(0,0,0)
    ...
    tabla_comparativa_linea_2(pdf, datos[0], datos_ant[0], 18, -1, cel, fill, "Heridos", color, amenaza)
    tabla_comparativa_linea_2(pdf, datos[1], datos_ant[1], 18, -1, cel, fill, "Asesinados", color, amenaza)
    pdf.ln(-10)
    pdf.cell(cel)
    pdf.cell(20,10,str("TOTAL"),1,0, 'C',fill)
...
def encabezado_comparativo_enemigo_dinamico_2(pdf, fill, datos, datos_ant, cel, color, suma, constante, texto, indicadores):
