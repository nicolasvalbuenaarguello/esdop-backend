...
def tabla_comparativa_linea( datos, datos_ant, id_enemigo, amenaza, suma, constante,  texto, indicadores, numero, data_ingreso,amenaza_1):
    dato_res = ''
    ...
    datos_r = [numero, amenaza_1, texto, datos_res[0], datos_res[1], datos_res[3], dato_res, signo]
    data_ingreso.append(datos_r)
    
    indicadores.append(datos_res[4])
    return [data_ingreso, indicadores, numero]

def tabla_comparativa_linea_2(datos, datos_ant, suma, constante,  texto, indicadores, numeros, data_registro):

    datos_res = calculo_comprativo_total(datos, datos_ant,  suma, constante )

    dato_res = ''

    if texto != "COMBATES SIN RESULTADOS" and texto != "TOTAL COMBATES"  and texto != "CAPTURAS LOE AMAZONÍA" and texto != "MADERA INCAUTADA (M3)"and texto != "ANIMALES INCAUTADOS" and texto != "PLÁNTULAS SEMBRADAS":
    ...
    datos_r = [numeros, 'TOTAL', texto, datos_res[0], datos_res[1], datos_res[3], dato_res, signo]
    data_registro.append(datos_r)
    
    return [data_registro, indicadores]
...
def encabezado_comparativo_enemigo_dinamico(
    datos, datos_ant, amenaza, suma, constante,
    id_enemigo, texto, indicadores, objectivos,
    rango, numeros, data_ingreso
):

    texto_evaluado = []
    indicador = [data_ingreso, indicadores]
    ...
    for idx in range(inicio, fin + 1):
        ...
        procesar = True
        if objectivos:
            procesar = objectivos[idx] == "SI"

        if procesar:

            indicador = tabla_comparativa_linea(
                datos[numero],
                datos_ant[numero],
                id_enemigo,
                amenaza[0],
    ...
    return [indicador[0], indicador[1], numeros, texto_evaluado]

  
  
    
def encabezado_comparativo_enemigo_dinamico_2(datos, datos_ant, suma, constante, texto, indicadores, objectivos, rango, numeros, data_registro):
    ...
    if objectivos !=[]:
        while i <= rango[1]:
            if objectivos[i] == "SI":
                ...
                if texto[numero] != "COMBATES SIN RESULTADOS" and texto[numero] != "TOTAL COMBATES" and texto[numero] != "CAPTURAS LOE AMAZONÍA" and texto[numero] != "MADERA INCAUTADA (M3)"and texto[numero] != "ANIMALES INCAUTADOS" and texto[numero] != "PLÁNTULAS SEMBRADAS":
                    indicador = tabla_comparativa_linea_2(datos[numero], datos_ant[numero], suma[numero], constante[numero], texto[numero] ,  indicadores, numeros, data_registro)
                    indicadores = indicador[1]
                    data_registro = indicador[0]
                    numeros +=1
                else:
                    indicador =tabla_comparativa_linea_2(datos[numero], datos_ant[numero], suma[numero], constante[numero], texto[numero] ,  indicadores, '-', data_registro)
                    data_registro = indicador[0]
    ...
    return [data_registro, indicadores, numeros]
