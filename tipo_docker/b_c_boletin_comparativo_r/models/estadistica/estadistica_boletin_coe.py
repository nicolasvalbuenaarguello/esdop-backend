...
from tipo_docker.b_c_boletin_comparativo_r.models.conexion_pos import Databa_bases
from tipo_docker.z_z_z_funtions.funciones import *
from tipo_docker.b_c_boletin_comparativo_r.models.funtions.componest.tablas import *
...
class Calculo_Spoa:
    ...
    def resultados_resaltantes_r(self, pdf, fecha_inicial_u_l, fecha_final_u_l, fecha_inicial_p_l, fecha_final_p_l, filtro, anio_act, anio_ant, fecha_titulo, fecha_titulo_dos):
        ...
        z = encabezado_comparativo(pdf, True, fecha_titulo_dos, fecha_titulo, 10)
        pdf.ln(-5)
        ...
        claculo_mapa_dos(pdf, valor_ant[0], valor_act[0], "LAB. PASTA O BASE DE COCA",1, 7, 18,5)
        ...
        claculo_mapa_dos(pdf, valor_ant[0], valor_act[0], "SEMILLEROS",1, 7, 18,5)
        ...
        claculo_mapa_dos(pdf, valor_ant[0], valor_act[0], "MATA(S) DE COCA EN SEMILLERO",1, 7, 18,5)
        ...
        claculo_mapa_dos(pdf, valor_ant[0], valor_act[0], "INSUMOS LIQUIDOS Gal",1, 7, 18,5)
        ...
        claculo_mapa_dos(pdf, valor_ant[0], valor_act[0], "INSUMOS SOLIDOS Kg",1, 7, 18,5)
        ...
        # claculo_mapa_dos(pdf, res_calculo_ante[5], res_calculo_act[5], "ASESINADOS",-1, 7, 18, 5)
        # claculo_mapa_dos(pdf, res_calculo_ante[6], res_calculo_act[6], "HERIDOS",-1, 7, 18, 5)
        ...
        claculo_maquinaria_amarialla_mapa_dos(pdf, maquinaria_anterior, maquinaria_anctual, "MAQUINARIA AMARILLA",1, 7, 18, 165)
        ...
        sepador_mapa_dos(pdf, True, "COMBATES", 165)
        claculo_mapa_dos(pdf, res_calculo_ante[11], res_calculo_act[11], "COMBATES POSITIVOS",1, 8, 14, 165)
        claculo_mapa_dos(pdf, res_calculo_ante[12], res_calculo_act[12], "COMBATES NEGATIVOS",1, 8, 14, 165)
        claculo_mapa_dos(pdf, res_calculo_ante[13], res_calculo_act[13], "COMBATES SIN RESULTADOS",1, 8, 14, 165)
        claculo_mapa_dos(pdf, res_calculo_ante[14], res_calculo_act[14], "TOTAL DE COMBATES",1, 8, 14, 165)
        
        sepador_mapa_dos(pdf, True, "CONTRABANDO", 165)
        ...
        claculo_mapa_dos(pdf, valor_ant[0], valor_act[0], "CAPTURAS",1, 8, 18, 165)
        claculo_mapa_dos(pdf, res_calculo_ante[50], res_calculo_act[50], "GASOLINA GAL",1, 8, 18, 165)
        claculo_mapa_dos(pdf, res_calculo_ante[51], res_calculo_act[51], "ACPM GAL",1, 8, 18, 165)
        claculo_mapa_dos(pdf, res_calculo_ante[52], res_calculo_act[52], "INUMOS LIQUIDOS GAL",1, 8, 18, 165)
        claculo_mapa_dos(pdf, res_calculo_ante[53], res_calculo_act[53], "VEHICULOS",1, 8, 18, 165)
        # claculo_mapa_dos(pdf, res_calculo_ante[54], res_calculo_act[54], "ANIMALES INCAUTADOS",1, 8, 18, 165)

        claculo_mapa_dos(pdf, res_calculo_ante[93], res_calculo_act[93], "LICORES",1, 8, 18, 165)
        claculo_mapa_dos(pdf, res_calculo_ante[94], res_calculo_act[94], "CIGARRILLOS",1, 8, 18, 165)
        claculo_mapa_dos(pdf, res_calculo_ante[95], res_calculo_act[95], "TELEFONOS",1, 8, 18, 165)
        claculo_mapa_dos(pdf, res_calculo_ante[96], res_calculo_act[96], "MUEBLES",1, 8, 18, 165)

        

        # sepador_mapa_dos(pdf, True, "DEPÓSITOS", 175)
        # claculo_mapa_dos(pdf, res_calculo_ante[46], res_calculo_act[46], "DEPÓSITO ILEGAL",1, 7, 18, 175) 
        ...
        if  filtro[4] == "lugar":
            if filtro[6]!="" and filtro[6]!="---" :#filtro por municipio
                ...
                if len(nueva) > 1:
                        ids = tuple(nueva)
                        dato = "and mpio in {}".format(ids)
                        dato_res= "and hop_mpio in {}".format(ids)

                else:
                    mpio = nueva[0]  
                    dato = " and {} = '{}'".format("mpio",mpio)
                    dato_res = " and {} = '{}'".format("hop_mpio",mpio)
        ...
        res_calculo_ante = estadistica_resultados(resultados_anterior, hechos_anterior, filtro)
        res_calculo_act = estadistica_resultados(resultados_actual, hechos_actual, filtro)
        datos = [res_calculo_ante[5], res_calculo_act[5], res_calculo_ante[6], res_calculo_act[6], anio_ant, anio_act, fecha_titulo, fecha_titulo_dos]
