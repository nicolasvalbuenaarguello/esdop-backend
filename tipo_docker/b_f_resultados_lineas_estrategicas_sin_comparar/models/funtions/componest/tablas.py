...
def tabla_comparativa_linea(pdf, datos, datos_ant, id_enemigo, amenaza, suma, constante, cel, fill, texto, color, indicadores):
    ...
    pdf.cell(cel+35+15)
    pdf.cell(100,5,texto,1,0, 'L',fill)
    pdf.cell(50.5,5,datos_res[1],1,0, 'C',fill)
...
def encabezado_comparativo_mapa(pdf, fill, anio_act, anio_ant, cel):
    ...
    pdf.cell(cel)
    pdf.cell(15,5,"No.",1,0, 'C',fill)
    pdf.cell(35,5,"AMENAZA",1,0, 'C',fill)
    pdf.cell(100,5,"EVENTO",1,0, 'C',fill)
    pdf.cell(50.5,5,str(anio_act),1,0, 'C',fill)
...
def encabezado_comparativo_enemigo_dinamico(pdf, fill, datos, datos_ant, amenaza, cel, color, suma, constante, id_enemigo, texto, indicadores, objectivos, rango, numeros):
    ...
    if altura >0:
        ...
        pdf.ln(-altura)
        pdf.cell(cel+15)
        pdf.cell(35,altura,str(amenaza[1]),1,0, 'C',fill)
        pdf.ln()
        ...
        for x in texto_evaluado:
            numeros = numeros +1
            pdf.cell(cel)
            pdf.cell(15,5,str(numeros),1,0, 'C',fill)
            pdf.ln()
...
def encabezado_comparativo_enemigo_dinamico_2(pdf, fill, datos, datos_ant, cel, color, suma, constante, texto, indicadores, objectivos, rango, numeros):
    ...
    i = rango[0] if len(rango) > 0 else 0
    numero = 0
    textos_len = []

    if objectivos:
        # escoger el límite de forma segura
        if len(rango) == 1:
            whilimite = rango[0]
        else:
            whilimite = rango[0] if texto == "COMBATES" else rango[1]

        while i <= whilimite and i < len(objectivos) and numero < len(texto):
            if objectivos[i] == "SI":
                pdf.set_line_width(0.1)
                pdf.set_fill_color(color[0], color[1], color[2])
                pdf.set_draw_color(0, 0, 0)
                ...
                if texto[numero] != "COMBATES SIN RESULTADOS":
                    indicadores_2 = tabla_comparativa_linea_2(
                        pdf, datos[numero], datos_ant[numero], suma[numero],
                        constante[numero], cel, fill, texto[numero], color, indicadores
                    )
                else:
                    tabla_comparativa_linea_2(
                        pdf, datos[numero], datos_ant[numero], suma[numero],
                        constante[numero], cel, fill, texto[numero], color, indicadores
                    )
    ...
    pdf.ln(-altura)
    pdf.cell(cel+15)
    pdf.cell(35,altura,str("TOTAL"),1,0, 'C',fill)
    pdf.ln()
