...
#calculo comparativo
def calculo_comprativo_total(datos_res_ant, datos_res_act, identificador_suma, constante,  unidad, posicion):
    numero_ant = 0
    numero_act = 0
    
    for dato in datos_res_ant:
        if  dato[posicion] == unidad:
            numero_ant =  numero_ant + float(dato[identificador_suma])
...
def calculo_comprativo_total_enemigo(datos_res_ant, datos_res_act, id_parametro, parametro, parametro_dos, identificador_suma, constante, posicion):
    numero_ant = 0
    numero_act = 0
    
    for dato in datos_res_ant:
        if dato[id_parametro] == parametro and dato[posicion] == parametro_dos:
            numero_ant =  numero_ant + float(dato[identificador_suma])
...
def encabezado_coe_div_compa(pdf, fill=True, unidad="UNIDAD"):
    pdf.cell(35)
    # --- Estilos ---
    pdf.set_line_width(0.3)
    pdf.set_fill_color(128, 0, 0)     # Rojo institucional
    pdf.set_draw_color(0, 0, 0)
    pdf.set_text_color(255, 255, 255)
    pdf.set_font('Arial', 'B', 10)

    alto = 8  # altura uniforme

    # --- Encabezado ---
    pdf.cell(56, alto, unidad, border=1, align='C', fill=fill)
    pdf.cell(35, alto, "Cant. Indicadores", border=1, align='C', fill=fill)
    pdf.cell(35, alto, "Superiores", border=1, align='C', fill=fill)
    pdf.cell(35, alto, "Iguales", border=1, align='C', fill=fill)
    pdf.cell(35, alto, "Inferiores", border=1, align='C', fill=fill)
...
def tabla_comparativa_linea(datos, datos_ant, id_enemigo, amenaza, suma, constante, indicadores, unidad,posicion):
    datos_res = calculo_comprativo_total_enemigo(datos, datos_ant, id_enemigo, amenaza, unidad, suma, constante, posicion)
    indicadores.append(datos_res[4])
    return indicadores

def tabla_comparativa_linea_2(datos, datos_ant, suma, constante, texto, indicadores, unidad, posicion):

    datos_res = calculo_comprativo_total(datos, datos_ant,  suma, constante,  unidad, posicion)
...
def encabezado_comparativo_enemigo_dinamico(datos, datos_ant, amenaza,  suma, constante, id_enemigo, texto, indicadores, objectivos, rango, unidad, posicion):
    ...
    if objectivos !=[]:
        while i <= rango[1]:
            if objectivos[i] == "SI":
                indicadores = tabla_comparativa_linea( datos[numero], datos_ant[numero], id_enemigo, amenaza[0], suma, constante, indicadores, unidad, posicion)
...
def encabezado_comparativo_enemigo_dinamico_2( datos, datos_ant, suma, constante, texto, indicadores, objectivos, rango, unidad, posicion):
        ...
        while i <= rango[1]:
            if objectivos[i] == "SI":
                ...
                if texto[numero] != "COMBATES SIN RESULTADOS":
                    indicadores = tabla_comparativa_linea_2(datos[numero], datos_ant[numero], suma[numero], constante[numero], texto[numero] , indicadores, unidad, posicion)
    

                else:
                    tabla_comparativa_linea_2(datos[numero], datos_ant[numero], suma[numero], constante[numero], texto[numero] ,  indicadores, unidad, posicion)
