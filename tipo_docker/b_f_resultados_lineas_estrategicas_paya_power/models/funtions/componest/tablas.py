...
def estadistica_resultados(datos_res, hechos, filtro):
    ...
    capturas_internas = list(filter(lambda res_capturas: "NO APLICA" in res_capturas[7], res_capturas))
    capturas_internas = list(filter(lambda capturas_internas: "RME" not in capturas_internas[9] , capturas_internas))
    ...
    #narcotrafico
    #capturas
    capturas_narcotrafico = list(filter(lambda capturas:  capturas[17] == "Narcotráfico" , capturas))
    capturas_narcotrafico_internas = list(filter(lambda capturas_internas:  capturas_internas[17] == "Narcotráfico" , capturas_internas))
...
def porcentajes_2(numero_ant, numero_act, constante):
    porcentaje = 0
    if numero_ant >0:
        if numero_act >0:
...
def comparativo_metas(datos_res_act, identificador_suma, constante, meta, text):
    ...
    porcentaje = porcentajes_2(meta, numero_act, constante)
...
def tabla_comparativa_linea_2( datos, datos_ant):

    datos_res = calculo_comprativo_total(datos, datos_ant,  18, 1)
    

    return [datos_res[0], datos_res[1], datos_res[2], datos_res[4]]
...
def encabezado_comparativo_mapa(pdf, fill, anio_act, anio_ant, cel):
    ...
    pdf.cell(cel)
    pdf.cell(50,5,"EVENTO",1,0, 'C',fill)
    pdf.cell(20,5,str(anio_ant),1,0, 'C',fill)
    pdf.cell(20,5,str(anio_act),1,0, 'C',fill)
    pdf.cell(25,5,"PORCENTAJE",1,0, 'C',fill)
    pdf.cell(25,5,"DIFERENCIA",1,0, 'C',fill)
    pdf.cell(27,5,"SEMAFORIZACIÓN",1,0, 'C',fill)
...
def encabezado_comparativo_enemigo_dinamico(pdf, fill, datos, datos_ant, amenaza, cel, color, suma, constante, id_enemigo, texto, indicadores):
    ...
    indicadores = tabla_comparativa_linea(pdf, datos[0], datos_ant[0], id_enemigo, amenaza[0], suma, constante, cel, fill,texto[0] , color, indicadores)
    indicadores = tabla_comparativa_linea(pdf, datos[1], datos_ant[1], id_enemigo, amenaza[0], suma, constante, cel, fill,texto[1] , color, indicadores)
    indicadores = tabla_comparativa_linea(pdf, datos[2], datos_ant[2], id_enemigo, amenaza[0], suma, constante, cel, fill,texto[2] , color, indicadores)
    indicadores = tabla_comparativa_linea(pdf, datos[3], datos_ant[3], id_enemigo, amenaza[0], suma, constante, cel, fill,texto[3] , color, indicadores)

    pdf.ln(-20)
    pdf.cell(cel)
    pdf.cell(20,20,str(amenaza[1]),1,0, 'C',fill)
    pdf.ln()

    return indicadores

    
def encabezado_comparativo_enemigo_dinamico_2(datos, datos_ant):
    ...
    for x in datos:
        ...
        indicadores = tabla_comparativa_linea_2( datos[numero], datos_ant[numero])
        numero = numero +1
        actual.append(indicadores[0])
        anteriror.append(indicadores[1])
        diferencia.append(indicadores[2])
        porcentaje.append(indicadores[3])


    return [actual,anteriror,diferencia,porcentaje]
