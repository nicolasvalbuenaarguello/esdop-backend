...
def calculo_comprativo_total(datos_res_ant, datos_res_act, identificador_suma, constante):
    ...
    return [numero_ant, numero_act, porcentaje, diferencia]
...
def encabezado_comparativo_mapa_2(pdf, fill, anio_act, anio_ant, anio_ant_tres, ancho):
    pdf.ln(6)
    pdf.set_line_width(0.1)
    ...
    pdf.multi_cell(36, 8, "ITEM", 1, "C", fill)
    pdf.ln(-8)
    pdf.cell(ancho+36)
    # pdf.multi_cell(17, 8, str(anio_ant_tres), 1, "C", fill)
    # pdf.ln(-8)
    # pdf.cell(ancho+36)
    pdf.multi_cell(20, 8, str(anio_ant), 1, "C", fill)
    pdf.ln(-8)
    pdf.cell(ancho+36+20)
    # pdf.multi_cell(15, 4, str(porc), 1, "C", fill)
    # pdf.ln(-8)
    # pdf.cell(ancho+36+17+17)

    pdf.multi_cell(20, 8, str(anio_act), 1, "C", fill)
    pdf.ln(-8)
    pdf.cell(ancho+36+20+20)
    porc = "DIF."
    pdf.multi_cell(15, 8, str(porc), 1, "C", fill)
    pdf.ln(-8)
    pdf.cell(ancho+36+20+20+15)
    porc = "%"
    pdf.multi_cell(15, 8, str(porc), 1, "C", fill)
...
def sepador_mapa(pdf, fill, text, cell, ln, acho):
    ...
    pdf.ln(ln)
    pdf.cell(cell)
    pdf.multi_cell(acho, 4, text, 1, "C", True)
...
def claculo_mapa_mapa_2(pdf, datos_res_ant,  datos_res_act, res_calculo_ante_tres, text, constante, id_enemigo, suma, ancho, ruta, altu_img, pos_img):
    # pdf.ln(5)
    ...
    pdf.multi_cell(36, 4, text, 1, "L", True)
    final = pdf.get_y()
    ...
    # pdf.ln(x*-1)
    # pdf.cell(ancho+36+17)
    pdf.multi_cell(20, x, maquinari_amarialla_anterior_2, 1, "C", True)
    pdf.ln(x*-1)
    pdf.cell(ancho+36+20)
    ...
    # pdf.ln(x*-1)
    # pdf.cell(ancho+36+27)
    pdf.multi_cell(20, x, maquinari_amarialla_actual, 1, "C", True)
    pdf.ln(x*-1)
    pdf.cell(ancho+36+20+20)
    
    pdf.multi_cell(15, x, diferencia, 1, "C", True)
    pdf.ln(x*-1)
    pdf.cell(ancho+36+20+20+15)
    ...
    pdf.multi_cell(15, x, porcentaje, 1, "R", True)
    pdf.ln(x*-1)
    pdf.set_text_color(0,0,0)
...
def claculo_mapa_2(pdf, datos_res_ant,  datos_res_act, res_calculo_ante_tres, text, constante, id_enemigo, suma, ancho, ruta, altu_img, pos_img):
    # pdf.ln(5)
    pdf.cell(ancho)
    pdf.set_text_color(0,0,0)
    pdf.set_fill_color(255, 255, 255)
    ...
    pdf.multi_cell(20, x, resultados[1], 1, "C", True)
    ...
    pdf.multi_cell(15, x, resultados[3], 1, "C", True)
    pdf.ln(x*-1)
    pdf.cell(ancho+36+20+20+15)
    ...
    if constante == 1:
        ...
        if resultados[2] < 0:
            ...
            pdf.set_text_color(232,172,2)
            triangulo = '{}static/img/tri_amarillo_new.png'.format(ruta)
        valor_porcentaje =  resultados[2] *-1
        porcentaje = formato_numero(valor_porcentaje)
        if valor_porcentaje >0:
            porcentaje = str("+ ") + str(porcentaje)
