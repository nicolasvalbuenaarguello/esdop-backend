...
def mapa_filtrado(dato, filtro):
    ...
    ruta_final = f'{filtro[15]}static/img/img_mapas/{unidad}_comparativo_div.png'
    plt.savefig(ruta_final, format="png", transparent=True, dpi=100)

#funcion para filtrar mapas cnsejos
...
def mapa_filtrado_dep(dato, filtro):
    ...
    # Recortar según departamentos
    municipios = municipios.clip(departamentos)
    point = point.clip(departamentos)

    # Agrupaciones por dpto y mpio
    agrup_dep = point.groupby("dpto")["dpto"].agg(["count"])
    point_dep = point.merge(agrup_dep, on="dpto")

    agrup_mpio = point.groupby("mpio")["mpio"].agg(["count"])
    point_mpio = municipios.merge(agrup_mpio, left_on="NOMBRE_ENT", right_on="mpio", how="left")

    # Join espacial
    join = sjoin(municipios, point_dep, how="left")
    join_1 = sjoin(departamentos, point_dep, how="left").boundary
    join_2 = sjoin(point, point_mpio, how="left")

    # Iniciar figura explícitamente para evitar errores de aspecto
    fig, ax = plt.subplots(figsize=(8, 8))

    # Dibujar capas base y puntos
    join_1.plot(ax=ax, color="black", alpha=0.5)
    join.boundary.plot(ax=ax, linewidth=0.3, color="silver")
    join_2.plot(ax=ax, color="black", alpha=0.1)
    point.plot(ax=ax, cmap="YlOrRd", markersize=30, alpha=0.5)

    # Ajuste dinámico de clases para cuantiles
    ...
    try:
        ...
        join_2.plot(ax=ax, color="orange", alpha=0.5)

    ax.axis('off')
    ax.set_aspect("equal", adjustable='box')  # Asegura aspecto finito y correcto
