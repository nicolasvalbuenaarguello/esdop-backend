...
def encabezado_comparativo_mapa_2(pdf, fill, anio_act, anio_ant, anio_ant_tres, ancho):
    ...
    pdf.ln(-8)
    pdf.cell(ancho+34+20+20)
    porc = "DIF."
    pdf.multi_cell(15, 8, str(porc), 1, "C", fill)
    pdf.ln(-8)
    pdf.cell(ancho+34+20+20+15)
    porc = "%"
    pdf.multi_cell(15, 8, str(porc), 1, "C", fill)
...
def sepador_mapa(pdf, fill, text, cell, ln, acho):
    ...
    pdf.ln(ln)
    pdf.cell(cell)
    pdf.multi_cell(acho, 5, text, 1, "C", fill)
...
def claculo_mapa_mapa_2(pdf, datos_res_ant,  datos_res_act, res_calculo_ante_tres, text, constante, id_enemigo, suma, ancho, ruta, altu_img, pos_img):
    ...
    pdf.multi_cell(34, 4, text, 1, "L", True)
    final = pdf.get_y()
