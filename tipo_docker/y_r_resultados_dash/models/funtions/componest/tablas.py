...
def tabla_comparativa_linea_2(datos, suma, constante, texto, img, estenxion):

    dato_enviar = ""
    datos_res = calculo_total_sin_filtro(datos, suma)
    dato_enviar = (datos_res, texto, img, estenxion)
    return dato_enviar
...
def sepador_mapa_dos(pdf, fill, text, cell, ancho):
    ...
    #pdf.set_fill_color(240,240,240)
def validar_indicador_unidad(json, json_ruta, obj):
        texto = []
        constante = []
        suma = []
        posicion = []
        ...
        with open(json_ruta, "rb") as read_file:
            ...
            for x in data:
                for y in data[x]:
                    if obj != "":# cuando este el filtro
                        if obj == y['APLICA']:# cuando este el filtro
        ...
        return [texto, constante, suma, posicion, spoa, img, estenxion]
...
def encabezado_comparativo_enemigo_dinamico_2(datos, suma, constante, texto, img, estenxion):
    ...
    while i < len(datos):
            ...
            indicadores_f = ""
            indicadores_f = tabla_comparativa_linea_2( datos[numero],  suma[numero], constante[numero], texto[numero], img[numero], estenxion[numero] )
                
            indicadores.append(indicadores_f)
