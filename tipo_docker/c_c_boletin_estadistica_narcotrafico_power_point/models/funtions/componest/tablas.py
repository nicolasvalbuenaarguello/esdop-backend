...
def calculo_sin_formato(datos_res, parametro, identificador, identificador_suma):
    numero = 0
...
def porcentajes(numero_ant, numero_act, constante):
    ...
    if numero_ant >0:
        if numero_act >0:
            porcentaje = numero_act/numero_ant *100 
        else:
            porcentaje = 0
    elif numero_act > 0:
        porcentaje = 100
...
# comparativo metas 
def comparativo_metas(TEXT, datos_res_act, identificador_suma, constante, meta, text):
    numero_act = 0
    
    # print(text)
    for dato in datos_res_act:
            if text == "EJÉRCITO NACIONAL":
                numero_act =  numero_act + float(dato[identificador_suma])
            elif text == dato[1]:
                numero_act =  numero_act + float(dato[identificador_suma])
    if numero_act == 0:
    ...
    numero_act_con = formato_numero(numero_act)
    color = ""
    if porcentaje >= 100:
        color = "pos_n"         # positivo
    elif porcentaje >= 80 and porcentaje < 99 :
        color = "neutro_n"      # neutral
    else:
        color = "neg_n"         # negativo
        
    numero_act =  formato_numero(porcentaje)
    resultados_f =  [TEXT, meta, numero_act_con, numero_act]
    return [resultados_f, color]

# comparativo metas 
def comparativo_metas_mes(TEXT, datos_res_act, identificador_suma, constante, meta, text, mes):
    numero_act = 0
    ...
    for dato in datos_res_act:
            if text == "EJÉRCITO NACIONAL":
                ...
                numero_act =  numero_act + float(dato[identificador_suma])
    meta_mes = ((meta/12)*mes)
    #protecion contra division de cero 
    if meta_mes == 0:
        porcentaje = 0
    else:
        porcentaje = porcentajes(meta_mes, numero_act, constante)
    ...
    numero_act =  formato_numero(porcentaje)
    meta_mes =  formato_numero(meta_mes)
    resultados_f =  [TEXT, meta, meta_mes, numero_act_con, numero_act]
    return [resultados_f, color]

def comparativo_metas_valores(datos_res_act, identificador_suma):
    numero_act = 0
...
def calculo_de_interdion_solo_encabezado(pdf,text,ln,alt,pos, anio):
    ...
    pdf.set_font('Arial', 'B', 10)
    pdf.cell(42,7,text,1,0, 'L',True)
    pdf.cell(17,7,"META",1,0, 'C',True)
    pdf.cell(17,7,anio,1,0, 'C',True)
...
def calculo_de_interdion_solo(TEXT, meta, datos_res_act, texto ):


    res  = comparativo_metas(TEXT, datos_res_act, 18, 1, meta, texto)
...
def encabezado_interdicion(pdf, ruta, ln, alt, pos, escudo, titulo, anio):
    ...
    pdf.ln(6)
    pdf.cell(ln)
    pdf.cell(42,5,titulo,1,0, 'C',True)
    pdf.cell(17,5,"META",1,0, 'C',True)
    pdf.cell(17,5,anio,1,0, 'C',True)
    pdf.cell(15,5,"%",1,0, 'C',True)


    ejc = '{}static/img/escudos/{}.jpg'.format(ruta, escudo)
  
    # C:\Users\nicolas.valbuena\Documents\programacion 2023\server\static\img\escudos\escudo_ejc.jpg
  
    pdf.image(ejc,alt,pos,13,13)
...
def calculo_de_interdion(text, meta, datos_res_act, datos_anterior,  texto):
...
def calculo_de_interdion_mes(text, meta, datos_res_act, datos_anterior,  texto, mes):


    res  = comparativo_metas_mes(text, datos_res_act, 18, 1, meta, texto, mes)

    return [res[0], res[1]]
    


def valores_interdicion(pdf, ruta):
    ejc = '{}static/img/escudos/mapa_n.png'.format(ruta)
    pdf.image(ejc,120,30.1,160,160) 


    tamanio = 30 #tamaño de la imagen
    tamanio_2 = 25 #tamaño de la imagen
    posicion = 10
    
    ejc = '{}static/img/escudos/COCAINA_new.png'.format(ruta)
    pdf.image(ejc,posicion,35,tamanio,tamanio_2) 


    ejc = '{}static/img/escudos/PBC_new.png'.format(ruta)
    pdf.image(ejc,posicion,64,tamanio,tamanio_2) 


    ejc = '{}static/img/escudos/MARIHUNA_new.png'.format(ruta)
    pdf.image(ejc,posicion,93,tamanio,tamanio_2) 


    ejc = '{}static/img/escudos/LAB_COCAINA_new.png'.format(ruta)
    pdf.image(ejc,posicion,122,tamanio,tamanio_2) 


    ejc = '{}static/img/escudos/LAB_PBC_new.png'.format(ruta)
    pdf.image(ejc,posicion,151,tamanio,tamanio_2) 


    ejc = '{}static/img/escudos/SEMILLEROS_new.png'.format(ruta)
    pdf.image(ejc,posicion,180,tamanio,tamanio_2) 
...
def valores_tabla_interdicion_semilleros(pdf, ln, datos_res_act, texto):
    texto_rojo = texto

    texto_NEGRO = texto + " (Incautada)"

    pdf.set_line_width(0.1)
    pdf.cell(ln)
    pdf.set_text_color(125,0,0)
    res  = comparativo_metas_valores(datos_res_act[0], 18)
    numero_act = formato_numero(res)
    pdf.set_font('Arial', 'B', 10)
    pdf.cell(50,5,texto_rojo,0,0, 'L',False)
    pdf.set_text_color(0,0,0)

    pdf.cell(25,5,numero_act,0,0, 'R',False)

    res  = comparativo_metas_valores(datos_res_act[1], 18)
    numero_act = formato_numero(res)
    pdf.ln()
    pdf.cell(ln)
    pdf.set_text_color(0,0,0)
    pdf.set_font('Arial', '', 10)
    pdf.cell(50,5,"Matas de coca en semillero",0,0, 'L',False)
    pdf.cell(25,5,numero_act,0,0, 'R',False)

    hect = res/10000
    docis = formato_numero(hect)

    pdf.ln()
    pdf.cell(ln)
    pdf.set_font('Arial', 'B', 10)
    pdf.cell(50,5,"Eqv. hectareas de cultivo ",0,0, 'L',False)
    pdf.cell(25,5,docis,0,0, 'R',False)
    
    pbc_hect = hect*7.9
    docis = formato_numero(pbc_hect)

    pdf.ln()
    pdf.cell(ln)
    pdf.set_text_color(0,0,0)
    pdf.set_font('Arial', '', 10)
    pdf.cell(50,5,"Eqv. kgs de cocaina ",0,0, 'L',False)
    pdf.cell(25,5,docis,0,0, 'R',False)
    
    pbc_docis = pbc_hect*1000
    docis = formato_numero(pbc_docis)
    pdf.ln()
    pdf.cell(ln)
    pdf.set_text_color(0,0,0)
    pdf.set_font('Arial', '', 10)
    pdf.cell(50,5,"Eqv.Dosis Personal (dos.)",0,0, 'L',False)
    pdf.cell(25,5,docis,0,0, 'R',False)

    pbc_docis = pbc_hect*1375
    docis = formato_numero(pbc_docis)
        
    pdf.ln()
    pdf.cell(ln)
    pdf.set_text_color(92,140,60)
    pdf.set_font('Arial', 'B', 10)
    pdf.cell(50,5,"Afectacion financiera",0,0, 'L',False)
    pdf.cell(25,5,docis,0,0, 'R',False)


    #----------------------------------------------------------- 

def valores_tabla_interdicion_marihuna(pdf, ln, datos_res_act, texto, valor ):
    texto_rojo = "Valor "+texto
    numero_act = formato_numero(valor)
    valor_text = "USD "+str(numero_act)
    ...
    pdf.cell(ln)
    pdf.set_text_color(125,0,0)
    res  = comparativo_metas_valores(datos_res_act, 18)
    numero_act = formato_numero(res)
    pdf.set_font('Arial', 'B', 10)
    pdf.cell(60,5,texto_rojo,0,0, 'L',False)
    pdf.cell(25,5,valor_text,0,0, 'R',False)

    pdf.ln()
    pdf.cell(ln)
    pdf.set_text_color(0,0,0)
    pdf.set_font('Arial', '', 10)
    pdf.cell(60,5,texto_NEGRO,0,0, 'L',False)
    pdf.cell(25,5,numero_act,0,0, 'R',False)

    #una docis marihuana son 20 gramos el cual se divide de un kilo dando 50 docis 
    docis = res*50
    docis = formato_numero(docis)
    #una docis marihuana son 20 gramos el cual se divide de un kilo dando 50 docis 

    pdf.ln()
    pdf.cell(ln)
    pdf.set_font('Arial', 'B', 10)
    pdf.cell(60,5,"Dosis personal (dos.) ",0,0, 'L',False)
    pdf.cell(25,5,docis,0,0, 'R',False)
    ...
    pdf.set_text_color(92,140,60)
    pdf.set_font('Arial', 'B', 10)
    pdf.cell(60,5,"Afectacion financiera ",0,0, 'L',False)
    pdf.cell(25,5,docis,0,0, 'R',False)
    #----------------------------------------------------------- 

    #----------------------------------------------------------- 

def valores_tabla_interdicion(datos_res_act):

    res  = comparativo_metas_valores(datos_res_act, 18)
    # numero_act = formato_numero(res)
    return res

def valores_tabla_interdicion_2(pdf, ln, datos_res_act, texto, valor ):
    texto_rojo = "Valor "+texto
    numero_act = formato_numero(valor)
    valor_text = "USD "+str(numero_act)

    texto_NEGRO = texto + " (Destruido)"

    pdf.set_line_width(0.1)
    pdf.cell(ln)
    pdf.set_text_color(125,0,0)
    res=0
    res_T  = comparativo_metas_valores(datos_res_act[0], 18)
    res = res+res_T
    res_T  = comparativo_metas_valores(datos_res_act[1], 18)
    res = res+res_T
    numero_act = formato_numero(res)
    pdf.set_font('Arial', 'B', 10)
    pdf.cell(60,5,texto_rojo,0,0, 'L',False)
