...
# comparativo metas 
def comparativo_metas(datos_res_act, identificador_suma, constante, meta, text):
    numero_act = 0
    ...
    return [numero_act_con, meta, porcentaje, numero_act]
...
def calculo_de_interdion_solo(pdf,text, meta, datos_res_act,  ln, texto, ruta,alt,pos, fills):

    pdf.set_line_width(0.1)
    pdf.cell(ln)
    pdf.set_text_color(0,0,0)

    res  = comparativo_metas(datos_res_act, 18, 1, meta, texto)
    ...
    pdf.cell(17,7,res[1],1,0, 'C',True)
    pdf.cell(17,7,res[0],1,0, 'C',True)
    porcentaje = formato_numero_un_decimal(res[2])
    pdf.cell(15,7,porcentaje,1,0, 'R',True)

    if res[2]> 75:
        ejc = '{}static/img/escudos/pos_n.png'.format(ruta)

    elif res[2]< 45:
        ejc = '{}static/img/escudos/neg_n.png'.format(ruta)
    else:
        ejc = '{}static/img/escudos/neutro_n.png'.format(ruta)
    pdf.image(ejc,alt,pos,4,4)
...
def calculo_de_interdion(pdf,text, meta, datos_res_act, datos_anterior,  ln, texto, ruta,alt,pos):
    ...
    res  = comparativo_metas(datos_res_act, 18, 1, meta, texto)
    res_ant  = comparativo_metas(datos_anterior, 18, 1, meta, texto)

    res_anterior_actu = round(res[3])
    res_anterior = round(res_ant[3])

    total = 0

    total = res_anterior_actu - res_anterior

    pdf.set_font('Arial', 'B', 10)
    pdf.cell(42,5,text,1,0, 'L',False)
    pdf.set_font('Arial', '', 10)
    pdf.cell(17,5,res[1],1,0, 'C',False)
    pdf.cell(17,5,res[0],1,0, 'C',False)
    porcentaje = formato_numero_un_decimal(res[2])
    pdf.set_fill_color(255, 255, 255)
    pdf.cell(15,5,porcentaje,1,0, 'R',True)
    pdf.set_fill_color(125, 0, 0)
    ...
    # pdf.image(triangulo,alt,pos,4,4)
