...
def informe_pendiente(datos):
    ...
    ruta = DIRECION
    unidad_g=""
    EVENTOS(datos, ruta, datos_final, unidad_g)
    mapa = '{}static/img/img_mapas/EVENTOS.png'.format(ruta)
    

    pdf.parametros(pie_pagina = "SI" , ruta = "C:/Users/nicolas.valbuena/Documents/programacion 2023/server/static/img/oficio/", imagen = "Diapositiva20.JPG", QR="C:/Users/nicolas.valbuena/Documents/programacion 2023/server/tipo_eventos_relevantes/a_a_guardar_evento/informe_pendiente_qr/QR.png", titulo= "EVENTOS OPERACIONALES", fecha_titulo= fecha_titulo,mapa=mapa)
...
def informe_pendiente_id(datos):
    ...
    for x  in data:
        unidad = x[3]
        if unidad=="DIV01" or unidad=="DIV02" or unidad=="DIV03" or unidad=="DIV04" or unidad=="DIV05" or unidad=="DIV06" or unidad=="DIV07" or unidad=="DIV08" or unidad=="FUTCO"or unidad=="FUTOM":
            if unidad=="FUTOM":
                unidad="FUTCO"
            unidad ="{}.JPG".format(unidad)
            unidad_png  = x[3]
            EVENTOS_id_2(datos, ruta, datos_final, unidad_png, unidad )
    ...
    pdf.parametros(pie_pagina = "SI" , ruta = "C:/Users/nicolas.valbuena/Documents/programacion 2023/server/static/img/divisiones_2025/", imagen = unidad, QR="C:/Users/nicolas.valbuena/Documents/programacion 2023/server/tipo_eventos_relevantes/a_a_guardar_evento/informe_pendiente_qr/QR.png", titulo= "EVENTO OPERACIONAL", fecha_titulo= fecha_titulo,mapa=mapa, tipo="mapa_evento", unidad_png = unidad_png)
    ...
    if data_2:
        ...
        pdf.parametros(pie_pagina = "SI" , ruta = "C:/Users/nicolas.valbuena/Documents/programacion 2023/server/static/img/oficio/", imagen = "Diapositiva18.JPG", QR="C:/Users/nicolas.valbuena/Documents/programacion 2023/server/tipo_eventos_relevantes/a_a_guardar_evento/informe_pendiente_qr/QR.png", titulo= "BITÁCORA", fecha_titulo= fecha_titulo,tipo="reporte")
...
def informe_pendiente_division(datos):
    #datos
    ...
    if fecha_evento and fecha_evento_final:
        ...
        filtro_fecha = ""
        fechas="1"
    if divi_padre !="null" and divi_padre:

        if fechas =="":
            filtro = "and divi_padre = '{}'".format(divi_padre)

        elif fechas=="1":
            filtro = "where divi_padre = '{}'".format(divi_padre)
...
def informe_pendiente_departamento(datos):
    ...
    if fecha_evento and fecha_evento_final:
        ...
        filtro_fecha = ""
        fechas="1"
    if departamento !="null" and departamento:

        if fechas =="":
            filtro = "and departamento = '{}'".format(departamento)

        elif fechas=="1":
            filtro = "where departamento = '{}'".format(departamento)
    else:
        filtro =""
    ...
    if tipo_evento !="null" and tipo_evento:
        ...
        if fechas =="":
            filtro_tipo_evento = "and tipo_evento = '{}'".format(tipo_evento)
        elif fechas=="1":
            filtro_tipo_evento = "where tipo_evento = '{}'".format(tipo_evento)
    else:
        filtro_tipo_evento =""
    ...
    cursor = conn.cursor()
    # query para traes el evento
    query="select * from registro_eventos_relevantes  {} {}  {} {} order by fecha_evento asc, hora_evento asc".format(filtro_fecha, filtro, filtro_3, filtro_tipo_evento)
    cursor.execute(query)
    data = cursor.fetchall()
    ...
    if departamento=="CAUCA" or  departamento=="VALLE DEL CAUCA"  or  departamento=="NARIÑO"  or  departamento=="ANTIOQUIA"  or  departamento=="AMAZONAS"  or  departamento=="ARAUCA" or  departamento=="BOLÍVAR":

        unidad ="{}.JPG".format(departamento)
        unidad_png  = departamento
        mapa_filtrado_dep(datos, ruta, datos_final, unidad_png, unidad )
        mapa = '{}static/img/img_mapas/{}.png'.format(ruta,unidad_png)
    ...
    pdf.parametros(pie_pagina = "SI" , ruta = "C:/Users/nicolas.valbuena/Documents/programacion 2023/server/static/img/departamentos/", imagen = unidad, QR="C:/Users/nicolas.valbuena/Documents/programacion 2023/server/tipo_eventos_relevantes/a_a_guardar_evento/informe_pendiente_qr/QR.png", titulo= "EVENTO OPERACIONAL", fecha_titulo= fecha_titulo,mapa=mapa, tipo="mapa_evento", unidad_png = unidad_png)
    ...
    for x in data:
        ...
        pdf.cell(position) 
        unidad = str(x[22]) 
        pdf.multi_cell(tamanio, 5, unidad, 0, "L", False)
        ...
        pdf.set_font('Arial Narrow', '', 10)
        unidad = str(x[23])
        pdf.multi_cell(tamanio, 5, unidad, 0, "J", False)
        pdf.cell(position) 
        pdf.set_text_color(125,0,0)
        unidad = str(x[20]) +" - "+ str(x[21])
        pdf.set_font('Arial Narrow', 'B', 8)
        ...
        pdf.set_line_width(1)
        pdf.set_draw_color(140, 140, 140)
        pdf.line(200, posicion_final-3, 335, posicion_final-3)
